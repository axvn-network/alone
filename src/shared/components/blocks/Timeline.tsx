...
function StatCard({
  ...
  return (
    <div className="rounded-lg border border-white/8 bg-AXVN-navy/70 p-4">
      ...
      <p className="mt-1 text-xs leading-relaxed text-AXVN-silver/60">
        {description}
      </p>
    </div>
  );
  ...
  return (
    <li className="relative flex items-start gap-5 sm:gap-7">
      ...
      <div className="relative z-10 flex flex-col items-center shrink-0 mt-1">
        <span
          className={`w-12 h-12 bg-AXVN-navy border rounded-full flex items-center justify-center text-xs font-semibold ${
            isReference
      ...
      {/* Card */}
      <article
        className={`flex-1 rounded-xl border bg-AXVN-deep transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/20 ${
          isReference
        ...
        >
          ...
          <div className="flex items-start gap-3">
            <div className="min-w-0 flex-1">
              <h3 className="font-semibold text-AXVN-ivory text-sm mb-1.5">
                {phase.title}
              </h3>
              <p className="text-AXVN-silver/70 text-xs leading-relaxed">
                {phase.detail}
              </p>
            </div>
            <ChevronDown
      ...
      >
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <StatCard
            ...
            description={
              selectedPhase?.title ?? "Chọn một giai đoạn để xem trọng tâm."
            }
          />
          <StatCard
            icon={Map}
        ...
        <div
          className="mt-5"
          aria-label={`Trình tự ${phases.length} giai đoạn tham chiếu`}
        >
          <div
            ...
            {phases.map((phase, index) => (
              <span
                ...
                className={`h-1.5 rounded-full transition-colors duration-300 ${
                  index < refPosition ? "bg-AXVN-gold/70" : "bg-AXVN-silver/20"
                }`}
              />
            ))}
          </div>
            ...
            {phases.map((phase) => {
              ...
              return (
                <button
                  ...
                  aria-controls={`timeline-phase-${phase.year}`}
                  className={`min-w-16 rounded-md border px-3 py-2 text-xs font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-AXVN-gold ${
                    isActive
