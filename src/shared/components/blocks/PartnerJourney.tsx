    ...
    options: [
      {
        ...
        result: {
          ...
          highlight:
            "Phù hợp nhất với cá nhân và tổ chức nhỏ muốn tham gia lịch sử",
          color: "text-sky-300",
      ...
      },
      {
        ...
        result: {
          ...
          highlight:
            "Điều kiện bắt buộc: lãi 2 năm liền, BCTC kiểm toán TP, chỉ 1 sàn TSMH",
      ...
      },
      {
        ...
        result: {
          ...
          group:
            "Vai trò kỹ thuật cốt lõi · Cổ phần hoán đổi theo định giá IP / sản phẩm",
          minCapital:
            "Góp bằng hệ thống & IP — không yêu cầu tiền mặt tối thiểu",
          maxEquity: "15–25% (hoán đổi IP + năng lực đội ngũ)",
          highlight:
            "Đây là vai trò QUAN TRỌNG NHẤT trong dự án. Không có hạ tầng CNTT cấp 4, không có giấy phép. Bạn là người biến dự án thành thực tế.",
...
const TECH_BREAKDOWN = [
  {
    ...
    detail: "Khớp lệnh mua/bán real-time · độ trễ <10ms",
  },
  {
    icon: Shield,
    label: "Bảo Mật CNTT Cấp 4",
    detail: "Chuẩn Bộ Công An · ISMS · Pen test · DR/BCP",
  },
  {
    icon: GitBranch,
    label: "10 Quy Trình Nghiệp Vụ",
    detail: "AML, KYC, Custody, Settlement, CBTT, Rủi ro...",
  },
  {
    icon: Layers,
    label: "Custody Wallet & KYC",
    detail: "Lưu ký TSMH · xác minh danh tính người dùng",
  },
];
...
export default function PartnerJourney() {
  ...
  const [selectedResult, setSelectedResult] = useState<NonNullable<
    Question["options"][0]["result"]
  > | null>(null);
  ...
  const handleSelect = (
    value: string,
    result?: Question["options"][0]["result"],
  ) => {
                  ...
                  >
                    <div
                      className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 transition-colors ${
                        isSelected
                          ? "bg-AXVN-gold/20"
                          : "bg-white/5 group-hover:bg-AXVN-gold/10"
                      }`}
                    >
                      <Icon
                        className={`w-4 h-4 transition-colors ${
                          isSelected
                            ? "text-AXVN-gold"
                            : "text-AXVN-silver/60 group-hover:text-AXVN-gold/70"
                        }`}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p
                        className={`font-semibold text-sm transition-colors ${
                          isSelected ? "text-AXVN-gold" : "text-AXVN-ivory"
                        }`}
                      >
                        {opt.label}
                      </p>
          ...
          >
            ...
            <div className="flex items-center gap-3 p-4 bg-emerald-500/8 border border-emerald-400/20 rounded-xl">
              ...
              <div>
                ...
                <p className="text-AXVN-silver/60 text-xs mt-0.5 leading-relaxed">
                  {FINAL_MESSAGE.desc}
                </p>
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {QUESTIONS.map((q) => {
                ...
                return (
                  <div
                    key={q.id}
                    className="p-3.5 bg-AXVN-deep border border-AXVN-gold/10 rounded-xl"
                  >
                    <p className="text-AXVN-gold/50 text-[10px] font-mono uppercase tracking-widest mb-1.5">
                      {q.label}
                    </p>
                    <p className="text-AXVN-ivory text-sm font-semibold leading-tight">
                      {selected.label}
                    </p>
                    <p className="text-AXVN-silver/65 text-[11px] mt-0.5">
                      {selected.detail}
                    </p>
                  </div>
                );
            ...
            {selectedResult && (
              <div className="p-5 bg-AXVN-navy border border-AXVN-gold/20 rounded-xl space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-AXVN-gold/70 text-[10px] font-mono uppercase tracking-widest">
                    Vai trò phù hợp nhất
                  </p>
                </div>
                <p className={`font-black text-base ${selectedResult.color}`}>
                  {selectedResult.role}
                </p>
                <p className="text-AXVN-silver/70 text-xs">
                  {selectedResult.group}
                </p>
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <div className="bg-AXVN-deep rounded-lg p-2.5">
                    <p className="text-AXVN-silver/60 text-[9px] uppercase tracking-widest mb-0.5">
                      Vốn tối thiểu
                    </p>
                    <p className={`font-bold text-xs ${selectedResult.color}`}>
                      {selectedResult.minCapital}
                    </p>
                  </div>
                  <div className="bg-AXVN-deep rounded-lg p-2.5">
                    <p className="text-AXVN-silver/60 text-[9px] uppercase tracking-widest mb-0.5">
                ...
                {answers["where"] === "tech-partner" && (
                  <div className="pt-2 space-y-2">
                    <p className="text-purple-300/80 text-[10px] font-mono uppercase tracking-widest">
                      Bạn cần xây dựng
                    </p>
                    <div className="grid sm:grid-cols-2 gap-2">
                      {TECH_BREAKDOWN.map((item) => {
                        ...
                        return (
                          <div
                            key={item.label}
                            className="flex items-start gap-2.5 bg-purple-500/5 border border-purple-500/15 rounded-lg p-2.5"
                          >
                            <Icon className="w-3.5 h-3.5 text-purple-400 shrink-0 mt-0.5" />
                            <div>
                              <p className="text-AXVN-ivory text-[11px] font-semibold leading-tight">
                                {item.label}
                              </p>
                              <p className="text-AXVN-silver/65 text-[10px] mt-0.5 leading-snug">
                                {item.detail}
                              </p>
                            </div>
                          </div>
