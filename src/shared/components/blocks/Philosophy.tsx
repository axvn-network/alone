...
export default function Philosophy() {
  ...
  const i18nCommitments = t("philosophy.commitments", lang) as unknown as
    { title: string; desc: string; number: string }[] | string;
  const fallbackCommitments = Array.isArray(i18nCommitments)
    ? i18nCommitments
    : [];
  const commitmentList =
    content.philCommitments &&
    Array.isArray(content.philCommitments) &&
    content.philCommitments.length > 0
      ? content.philCommitments
      : fallbackCommitments;
      ...
      <div className="relative max-w-[1400px] mx-auto section-px">
        <div className="max-w-[900px] mx-auto">
          <Stagger>
            ...
            <StaggerItem>
              <h2
                className="text-AXVN-ivory text-center font-light mb-7 md:mb-9 uppercase leading-[1.28]"
                style={{
            ...
            <StaggerItem>
              <p
                className="text-AXVN-silver/80 leading-[1.8] text-center max-w-[680px] mx-auto mb-12 md:mb-16"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                {commitmentList.map((item, i) => (
                  <motion.div
                    ...
                    initial="rest"
                    whileHover="hover"
                    className="text-center border bg-AXVN-deep rounded-sm cursor-default"
                    style={{ padding: "var(--card-padding)" }}
                  >
                    <motion.span
                      className="text-AXVN-gold/30 font-thin block mb-5 leading-none tracking-tight"
                    ...
                    <h3
                      className="font-semibold text-AXVN-ivory mb-4 leading-[1.4]"
                      style={{ fontSize: "var(--text-h3)" }}
                    >
                      {item.title}
                    </h3>
                    <p
                      className="text-AXVN-silver/70 leading-[1.8]"
            ...
            <StaggerItem>
              <div className="mt-10 md:mt-14 flex justify-center">
                <p
                  className="text-AXVN-silver/45 leading-[1.8] text-center max-w-[560px] italic border-t border-AXVN-gold/10 pt-8"
                  style={{ fontSize: "var(--text-body)" }}
                >
                  {philQuote}
                </p>
