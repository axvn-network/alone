    ...
    <div className="w-full">
      ...
      <div className="block lg:hidden space-y-3">
        ...
        <div className="flex justify-center">
          ...
          >
            <p
              className={`${TEXT_SIZE.lg} ${NODES[0].textColor} tracking-widest`}
            >
              {NODES[0].label}
            </p>
            <p className={`${SUB_SIZE.lg} text-AXVN-silver/50 mt-0.5`}>
              {NODES[0].sub}
            </p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {NODES.slice(1).map((node, i) => (
            ...
            >
              <p
                className={`${TEXT_SIZE.sm} ${node.textColor} tracking-wide leading-tight`}
              >
                {node.label}
              </p>
              <p
                className={`${SUB_SIZE.sm} text-AXVN-silver/65 mt-1 leading-snug`}
              >
                {node.sub}
              </p>
            </motion.div>
          ...
          {CONNECTIONS.map((c, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <p className="text-AXVN-silver/70 text-[11px]">
                <span className="text-AXVN-gold/80 font-semibold">
                  AXVN Tech Holding
                </span>
                {" → "}
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-x-6 gap-y-2">
        {[
          ...
          <div key={item.label} className="flex items-center gap-1.5">
            ...
            />
            <span className="text-AXVN-silver/70 text-[10px]">
              {item.label}
            </span>
          </div>
        ))}
