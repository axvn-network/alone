...
export default function WhyChooseUs() {
  ...
  const i18nBenefits = t("why.benefits", lang) as unknown as
    { title: string; desc: string }[] | string;
  const fallbackBenefits = Array.isArray(i18nBenefits) ? i18nBenefits : [];
  const benefitList =
    content.whyBenefits &&
    Array.isArray(content.whyBenefits) &&
    content.whyBenefits.length > 0
      ? content.whyBenefits
      : fallbackBenefits;
    ...
    >
      ...
      <div className="relative max-w-[1400px] mx-auto section-px">
        <Stagger>
          ...
          <StaggerItem>
            <h2
              className="font-light text-AXVN-ivory mb-10 md:mb-14 leading-[1.28] uppercase"
              style={{
          ...
          <StaggerItem>
            <div className="grid sm:grid-cols-2 gap-x-10 md:gap-x-16 gap-y-7 md:gap-y-10">
              {benefitList.map((item, i) => (
                ...
                >
                  <motion.div
                    className="h-px w-8 bg-AXVN-gold/30 mb-4"
                    whileHover={{
                      width: "3rem",
                      backgroundColor: "rgba(201,162,74,0.7)",
                    }}
                    transition={{ duration: 0.3, ease: "easeOut" }}
                  />
                  <h3
                    className="font-semibold text-AXVN-gold mb-3 transition-colors duration-300 group-hover:text-AXVN-champagne leading-[1.4]"
                    style={{ fontSize: "var(--text-h3)" }}
                  >
                    {item.title}
                  </h3>
                  <p
                    className="text-AXVN-silver/65 leading-[1.8] transition-colors duration-300 group-hover:text-AXVN-silver/85"
