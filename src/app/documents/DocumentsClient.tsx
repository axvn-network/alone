...
/* ── Config ─────────────────────────────────────────────────── */
const TABS: { key: DocumentCategory | "all"; label: string; icon: ReactNode }[] = [
  { key: "all", label: "Tất cả tài liệu", icon: <FileText className="w-4 h-4" /> },
  { key: "annual_report", label: "Báo cáo thường niên", icon: <BookOpen className="w-4 h-4" /> },
  { key: "financial_report", label: "Báo cáo tài chính", icon: <BarChart2 className="w-4 h-4" /> },
  { key: "governance_report", label: "Báo cáo quản trị", icon: <ClipboardList className="w-4 h-4" /> },
  { key: "disclosure", label: "Công bố thông tin", icon: <Megaphone className="w-4 h-4" /> },
  { key: "charter", label: "Điều lệ & Quy chế", icon: <ScrollText className="w-4 h-4" /> },
  { key: "shareholder_meeting", label: "Đại hội cổ đông", icon: <Users className="w-4 h-4" /> },
];
...
function FileTypeBadge({ type }: { type: string }) {
  ...
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 border rounded uppercase ${map[type] || map.other}`}>
      {type}
    </span>
  );
}
...
function FinancialMatrix({ docs, years, selectedYear }: { docs: DocItem[]; years: number[]; selectedYear: number | null }) {
  const filteredDocs = selectedYear ? docs.filter((d) => d.year === selectedYear) : docs;
  ...
  const hasAny = (rt: string) => matrixYears.some((yr) => Object.values(matrix[rt][yr] || {}).length > 0);
  const totalDocs = Object.values(matrix).reduce((sum, byYr) =>
    sum + Object.values(byYr).reduce((s2, byQ) => s2 + Object.values(byQ).length, 0), 0);

  if (totalDocs === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <BarChart2 className="w-12 h-12 text-gray-200 mb-4" />
        <p className="text-gray-500 font-medium mb-1">Chưa có báo cáo tài chính nào</p>
      ...
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-[#07111D] text-white">
            <th className="text-left px-4 py-3 font-semibold text-xs whitespace-nowrap min-w-[220px]">Loại báo cáo</th>
            {matrixYears.map((yr) => (
              <th key={yr} colSpan={4} className="px-2 py-3 text-center font-semibold text-xs border-l border-white/10">
                {yr}
              </th>
            ))}
          </tr>
          <tr className="bg-[#0d1e30] text-gray-400 text-[11px]">
            ...
            {matrixYears.map((yr) =>
              QUARTERS.map((q) => (
                <th key={`${yr}-${q}`} className="px-2 py-2 text-center font-medium border-l border-white/5 whitespace-nowrap">
                  {q}
                </th>
              ))
            )}
        ...
        <tbody>
          {Object.entries(REPORT_TYPE_LABELS).map(([rt, label]) => {
            ...
            return (
              <tr key={rt} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3 font-medium text-gray-800 text-xs leading-snug">{label}</td>
                {matrixYears.map((yr) =>
                  ([1, 2, 3, 4] as const).map((q) => {
                    const doc = matrix[rt][yr]?.[q];
                    return (
                      <td key={`${yr}-${q}`} className="px-2 py-3 text-center border-l border-gray-100">
                        {doc ? (
                          <a
                            href={doc.fileUrl}
                            target="_blank"
...
function DocTable({ docs, total, page, pageSize, onPage, search, onSearch, onPageSize }: {
  docs: DocItem[];
  total: number;
  page: number;
  pageSize: number;
    ...
    <div className="space-y-4">
        ...
        <table className="w-full text-sm">
            ...
            <tr className="bg-[#07111D] text-white">
              <th className="text-left px-5 py-3 font-semibold text-xs">
                <span className="flex items-center gap-1.5">
                  <ScrollText className="w-3.5 h-3.5 text-[#C9A24A]" /> Tên tài liệu
                </span>
              </th>
          ...
          <tbody>
            {docs.length === 0 ? (
              <tr>
                <td colSpan={2} className="text-center py-16">
                  <div className="flex flex-col items-center gap-3 text-gray-400">
                    ...
                    <p className="font-medium text-gray-500">Không có tài liệu nào phù hợp.</p>
              ...
              docs.map((doc) => (
                <tr key={doc._id} className="border-b border-gray-100 hover:bg-amber-50/30 transition-colors group">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {doc.isFeatured && (
                            <span className="text-[#C9A24A] text-xs shrink-0">⭐</span>
                          )}
                        ...
                        <p className="text-[11px] text-gray-400 mt-0.5 flex items-center gap-1">
                          <Calendar className="w-3 h-3 text-[#C9A24A]" /> {fmtDate(doc.publishedDate)}
                        </p>
                      </div>
                    </div>
                  </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-gray-500">
          <p>
            Hiển thị {Math.min((page - 1) * pageSize + 1, total)} tới {Math.min(page * pageSize, total)} của{" "}
            {total} dữ liệu
          </p>
...
/* ── Year selector ───────────────────────────────────────────── */
function YearSelector({ years, selected, onChange }: { years: number[]; selected: number | null; onChange: (y: number | null) => void }) {
  ...
  return (
    <div>
      ...
      <PageHero
        ...
        description="Fortress Investment Holdings cam kết công bố đầy đủ, kịp thời và minh bạch mọi thông tin liên quan đến hoạt động kinh doanh, tài chính và quản trị doanh nghiệp."
      />
        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-8 lg:px-12">
          ...
          {loading ? (
            ...
            <div className="space-y-8">
              <FinancialMatrix docs={docs} years={years} selectedYear={selectedYear} />
            </div>
          ) : (
            <DocTable
              docs={docs}
