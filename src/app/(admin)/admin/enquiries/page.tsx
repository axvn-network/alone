    ...
    <div className="min-h-screen bg-[#03080e] flex selection:bg-AXVN-gold/20 selection:text-AXVN-champagne font-sans">
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden min-h-screen relative">
        ...
        <div className="flex flex-col md:flex-row flex-1 min-h-[calc(100vh-64px)] relative z-10">
          <div
            className={`w-full md:w-[420px] border-b md:border-b-0 md:border-r border-AXVN-gold/10 flex flex-col bg-[#07111D]/80 backdrop-blur-xl ${selected ? "hidden md:flex" : ""}`}
          >
            <div className="p-4 border-b border-AXVN-gold/10">
              <div className="flex items-center justify-between mb-3">
                {unread > 0 && (
                  <span className="text-[10px] bg-AXVN-gold/15 text-AXVN-gold font-bold px-2 py-0.5 rounded-full tracking-wide">
                    {unread} chưa đọc
                  </span>
                )}
              </div>
              <div className="flex gap-1 p-1 bg-AXVN-deep rounded-lg">
                {(["all", "contact", "submission"] as const).map((f) => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`flex-1 px-3 py-1.5 text-xs font-medium transition-colors tracking-wide rounded-md ${filter === f ? "bg-AXVN-navy text-AXVN-ivory shadow-sm" : "text-AXVN-silver/60 hover:text-AXVN-ivory hover:bg-AXVN-gold/5"}`}
                  >
                    {f === "all"
                      ? "Tất cả"
                      : f === "contact"
              ...
              {filtered.map((e) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <p
                      className={`text-sm truncate ${selected?.id === e.id ? "font-semibold text-AXVN-ivory" : !e.read ? "font-semibold text-AXVN-ivory" : "font-medium text-AXVN-silver/80"}`}
                    >
                      {e.name}
                    </p>
                    <span className="text-[10px] text-AXVN-silver/50 whitespace-nowrap">
                      {timeAgo(e.createdAt)}
                    </span>
                  </div>
                  <p
                    className={`text-xs truncate mt-0.5 ${selected?.id === e.id ? "text-AXVN-silver/70" : "text-AXVN-silver/50"}`}
                  >
                    {e.subject}
                  </p>
                  <div className="flex items-center gap-2 mt-1">
                    <span
                      className={`text-[10px] uppercase tracking-wider ${selected?.id === e.id ? "text-AXVN-gold/70" : "text-AXVN-silver/40"}`}
                    >
                      {e.type}
                    </span>
                    {!e.read && (
                      <span className="w-1.5 h-1.5 bg-AXVN-gold rounded-full" />
                    )}
                  </div>
                </button>
              ))}
              {filtered.length === 0 && (
                <p className="text-center text-AXVN-silver/50 text-xs py-10">
          ...
          <div
            className={`flex-1 p-5 md:p-8 overflow-auto bg-[#03080e] ${!selected ? "hidden md:flex md:items-center md:justify-center" : ""}`}
          >
            {selected ? (
              <div className="max-w-2xl mx-auto space-y-6 w-full">
                <button
                  onClick={() => setSelected(null)}
                  className="md:hidden flex items-center gap-1.5 text-AXVN-silver hover:text-AXVN-ivory text-xs transition-colors mb-3 rounded-md"
                >
                  <svg
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <a
                      ...
                      className="text-AXVN-gold text-sm hover:text-AXVN-champagne inline-flex items-center gap-1.5 mt-1"
                    >
                      <Mail className="w-4 h-4" /> {selected.email}
                    </a>
                    <p className="text-AXVN-silver/70 text-xs mt-3 capitalize flex items-center gap-2">
                      <span className="px-2 py-1 bg-AXVN-deep border border-AXVN-gold/20 rounded-md text-AXVN-silver/80 text-[10px] font-medium tracking-wide">
                        {selected.type}
                      </span>
                      {selected.subject}
                    </p>
                  ...
                  <div className="flex items-center gap-1 bg-AXVN-deep border border-AXVN-gold/10 rounded-lg p-1">
                    {!selected.read && (
                      <button
                        ...
                        className="p-2 text-AXVN-silver/60 hover:text-AXVN-gold transition-colors rounded-md hover:bg-AXVN-gold/5"
                    ...
                    <button
                      ...
                      className="p-2 text-AXVN-silver/60 hover:text-red-400 transition-colors rounded-md hover:bg-red-500/10"
                      title="Xóa"
                    >
                ...
                <div className="bg-AXVN-deep/60 border border-AXVN-gold/10 rounded-xl p-6">
                  <p className="text-AXVN-silver/80 text-sm leading-relaxed whitespace-pre-wrap">
                    {selected.message}
                  </p>
                </div>
                ...
                {selected.details?.document && (
                  ...
                  >
                    ...
                    <div>
                      <span className="block text-sm font-medium text-AXVN-ivory">
                        Xem tài liệu đính kèm
                      </span>
                      <span className="block text-xs text-AXVN-silver/60 mt-0.5">
                        Mở trong tab mới
                      </span>
                    </div>
                  </a>
              ...
              <div className="text-center max-w-sm mx-auto">
                ...
                <h3 className="text-AXVN-ivory font-medium mb-1">
