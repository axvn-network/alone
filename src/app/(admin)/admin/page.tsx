...
const statCards = [
  ...
  },
  {
    ...
    color: "from-AXVN-gold/20 to-AXVN-gold/5",
    iconColor: "text-AXVN-gold",
    borderColor: "border-AXVN-gold/20",
  },
  {
...
const quickActions = [
  ...
  },
  {
    label: "Giao Dịch Vốn",
    icon: BarChart3,
    href: "/admin/capital-transactions",
        ...
        <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {statCards.map((card) => (
              ...
              >
                <div
                  className={`absolute top-0 inset-x-0 h-0.5 bg-gradient-to-r ${card.color}`}
                />
                <div className="p-5">
                  <div
                    className={`w-9 h-9 rounded-xl bg-gradient-to-br ${card.color} flex items-center justify-center mb-4`}
                  >
                  ...
                  <p className="text-[11px] font-semibold text-AXVN-ivory/80 mt-0.5">
                    {card.label}
                  </p>
                  <p className="text-[10px] text-AXVN-silver/40 mt-0.5 truncate">
                    {card.sub}
                  </p>
                </div>
              </Link>
            ...
            <div className="lg:col-span-2 bg-[#07111D]/70 backdrop-blur-xl rounded-2xl border border-white/6 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-AXVN-ivory">
                      Hoạt Động Gần Đây
                    </h3>
                    <p className="text-[11px] text-AXVN-silver/40">
              ...
              <div className="flex-1 divide-y divide-white/4">
                {stats.activities.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="text-sm text-AXVN-silver/40">
                  ...
                  stats.activities.slice(0, 6).map((a) => (
                    ...
                    >
                      <div
                        className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${a.type === "contact" ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-AXVN-gold/10 border border-AXVN-gold/20"}`}
                      >
                        {a.type === "contact" ? (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-AXVN-silver/50 truncate mt-0.5">
                          {a.description}
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        <span
                          className={`text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase tracking-wider ${a.type === "contact" ? "bg-emerald-500/10 text-emerald-400" : "bg-AXVN-gold/10 text-AXVN-gold"}`}
                        >
