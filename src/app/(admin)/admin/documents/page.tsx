...
import AiAssistPanel, { DOC_AI_ACTIONS } from "@/app/(site)/components/ui/AiAssistPanel";
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto">
      ...
      <div className="relative bg-[#07111D] border border-white/10 rounded-2xl shadow-2xl w-full max-w-lg p-6 my-8">
        ...
        <div className="space-y-4">
          {/* Title */}
          <div>
            <label className="text-xs font-semibold text-AXVN-silver/60 uppercase tracking-wide mb-1.5 block">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-AXVN-ivory placeholder:text-AXVN-silver/30 focus:outline-none focus:border-AXVN-gold/40"
          ...
          <label className="flex items-center gap-2.5 cursor-pointer">
            ...
            />
            <span className="text-sm text-AXVN-silver/70">⭐ Nổi bật</span>
          </label>
    ...
    <div className="min-h-screen bg-[#03080e] flex selection:bg-AXVN-gold/20 selection:text-AXVN-champagne font-sans">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-AXVN-gold text-AXVN-navy text-sm font-bold hover:bg-AXVN-champagne transition-all rounded-lg shadow-lg shadow-AXVN-gold/10"
              >
          ...
          <div className="bg-[#07111D]/60 backdrop-blur-xl border border-white/5 rounded-xl overflow-hidden shadow-2xl shadow-black/30">
            ...
            <div className="px-5 py-4 border-b border-white/5 flex flex-col sm:flex-row items-start sm:items-center gap-3">
              <div className="relative flex-1 max-w-sm">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-AXVN-silver/40" />
                <input
                  type="text"
                  placeholder="Tìm kiếm tài liệu..."
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                  className="w-full pl-9 pr-4 py-2 bg-white/5 border border-white/10 text-AXVN-ivory text-sm rounded-lg placeholder:text-AXVN-silver/30 focus:outline-none focus:border-AXVN-gold/40"
                />
              </div>
              <div className="flex items-center gap-1 bg-white/5 rounded-lg p-1 overflow-x-auto w-full sm:w-auto">
                <button
                  onClick={() => setFilterCat("all")}
                  className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${filterCat === "all" ? "bg-AXVN-gold text-AXVN-navy shadow" : "text-AXVN-silver/50 hover:text-AXVN-ivory"}`}
                >
                  Tất cả ({docs.length})
                </button>
                {CATEGORIES.map(([k, v]) => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all whitespace-nowrap ${filterCat === k ? "bg-AXVN-gold text-AXVN-navy shadow" : "text-AXVN-silver/50 hover:text-AXVN-ivory"}`}
                  >
                    ...
                    {filtered.map((doc) => (
                      <tr key={doc._id} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors group">
                        ...
                        <td className="px-4 py-4 hidden sm:table-cell">
                          <span className="text-[10px] font-semibold px-2 py-0.5 bg-AXVN-gold/10 text-AXVN-gold border border-AXVN-gold/20 rounded-full">
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <a
          href="/documents"
          target="_blank"
          className="flex items-center gap-2 px-4 py-2.5 bg-[#07111D] border border-AXVN-gold/30 text-AXVN-gold text-xs font-semibold rounded-xl hover:bg-AXVN-gold/10 transition-all shadow-xl"
        >
