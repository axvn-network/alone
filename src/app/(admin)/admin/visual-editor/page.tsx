  ...
  return (
    <div className="mb-4">
      <label className="block text-AXVN-silver text-xs font-medium mb-1.5 tracking-wide">
        {label}
        {hint && (
          <span className="ml-2 text-AXVN-silver/40 font-normal normal-case">
            {hint}
          </span>
        )}
      </label>
  ...
  return (
    <div className="mb-4">
      <div className="flex items-center justify-between mb-2">
        <span className="text-AXVN-silver text-xs font-medium tracking-wide">
          {label}
        </span>
        <button
          onClick={add}
      ...
      <div className="space-y-3">
        {items.map((item, idx) => (
          <div
            key={idx}
            className="bg-[#070e1a] border border-white/8 rounded-lg p-3 relative group"
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-AXVN-gold/50 text-xs font-mono">
  ...
  return (
    <div className="bg-AXVN-navy border border-AXVN-gold/10 rounded-xl overflow-hidden mb-4">
      ...
      >
        ...
        {open ? (
          <ChevronUp className="w-4 h-4 text-AXVN-silver/40" />
        ) : (
          <ChevronDown className="w-4 h-4 text-AXVN-silver/40" />
        )}
      </button>
      {open && (
        <div className="px-5 pb-5 pt-1 border-t border-AXVN-gold/5">
          {children}
        </div>
      )}
    </div>
...
export default function VisualEditorPage() {
  ...
  const [data, setData] = useState<{ vi: SectionState; en: SectionState }>({
    vi: {},
    en: {},
  });
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 relative z-10">
          ...
          {loading ? (
            <div className="flex items-center justify-center py-24 text-AXVN-silver/40 text-sm">
              <RefreshCw className="w-5 h-5 animate-spin mr-2" /> Đang tải dữ
              liệu...
            </div>
          ) : (
            <div className="space-y-2">
              {/* ══════════════════════ HERO ══════════════════════ */}
              <SectionCard
                title="Hero — Màn hình đầu trang"
                badge="01"
                defaultOpen
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-3">
                  <Field
                    label="Thẻ nhãn (eyebrow)"
                    hint="Ví dụ: AXVN Tech Holding"
                    value={get("heroSubtitle")}
                    onChange={(v) => set("heroSubtitle", v)}
                  />
                  <Field
                    label="Tiêu đề dòng 1"
                    hint="Dòng trắng (font nhẹ)"
                    value={get("heroTitleLine1")}
                    onChange={(v) => set("heroTitleLine1", v)}
                  />
                  <Field
                    label="Tiêu đề dòng 2 (vàng gradient)"
                    hint="Dòng vàng — in đậm"
                    value={get("heroTitleLine2")}
                    onChange={(v) => set("heroTitleLine2", v)}
                  />
                  <Field
                    label="Nút CTA 1"
                    hint="Nút chính (vàng)"
                    value={get("heroBtn1Text")}
                    onChange={(v) => set("heroBtn1Text", v)}
                  />
                  <Field
                    label="Nút CTA 2"
                    hint="Nút phụ (viền)"
                    value={get("heroBtn2Text")}
                    onChange={(v) => set("heroBtn2Text", v)}
                  />
                </div>
                <Field
                  label="Mô tả Hero"
                  hint="Đoạn văn giới thiệu ngắn dưới tiêu đề"
                  multiline
                  rows={3}
                  value={get("heroDescription")}
              ...
              <SectionCard title="Introduction — Giới thiệu" badge="02">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-3">
                  <Field
                    label="Thẻ nhãn section"
                    hint="Ví dụ: 01 — Introduction"
                    value={get("introTag")}
                    onChange={(v) => set("introTag", v)}
                  />
                  <Field
                    label="Tiêu đề section"
                    value={get("introTitle")}
                ...
                <Field
                  ...
                  multiline
                  rows={3}
                  value={get("introParagraph1")}
                  onChange={(v) => set("introParagraph1", v)}
                />
                <Field
                  label="Đoạn văn 2"
                  multiline
                  rows={3}
                  value={get("introParagraph2")}
                  onChange={(v) => set("introParagraph2", v)}
                />
                <Field
                  label="Đoạn văn 3"
                  multiline
                  rows={3}
                  value={get("introParagraph3")}
                  onChange={(v) => set("introParagraph3", v)}
                />
              </SectionCard>
              ...
              <SectionCard title="What We Do — Dịch vụ cốt lõi" badge="03">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-3">
                  <Field
                    label="Thẻ nhãn section"
                    hint="Ví dụ: 02 — Core Capabilities"
                    value={get("whatTag")}
                    onChange={(v) => set("whatTag", v)}
                  />
                  <Field
                    label="Tiêu đề section"
                    value={get("whatTitle")}
                  ...
                  />
                  <Field
                    label="Tagline phụ (vàng)"
                    value={get("whatSubtitle")}
                    onChange={(v) => set("whatSubtitle", v)}
                  />
                </div>
                <Field
                  label="Đoạn mô tả 1"
              ...
              {/* ══════════════════════ INVESTMENT SECTORS ══════════════════════ */}
              <SectionCard
                title="Investment Sectors — Lĩnh vực đầu tư"
                badge="04"
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-3">
                  <Field
                    label="Thẻ nhãn section"
                    hint="Ví dụ: 03 — Investment Sectors"
                    value={get("sectorsTag")}
                    onChange={(v) => set("sectorsTag", v)}
                  />
                  <Field
                    label="Tiêu đề section"
                    value={get("sectorsTitle")}
                    onChange={(v) => set("sectorsTitle", v)}
                  />
                  <Field
                    label="Nút CTA xem tất cả"
                    value={get("sectorsBtnText")}
                    onChange={(v) => set("sectorsBtnText", v)}
                  />
                </div>
                <Field
                  label="Đoạn mô tả"
                  multiline
                  rows={3}
                  value={get("sectorsDesc")}
                  onChange={(v) => set("sectorsDesc", v)}
                />
                <ListEditor
              ...
              <SectionCard title="Why Choose Us — Tại sao chọn AXVN" badge="05">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-3">
                  <Field
                    label="Thẻ nhãn section"
                    hint="Ví dụ: 04 — Why Choose AXVN Tech Holding"
                    value={get("whyTag")}
                    onChange={(v) => set("whyTag", v)}
                  />
                  <Field
                    label="Tiêu đề section"
                    value={get("whyTitle")}
              ...
              <SectionCard title="Philosophy — Triết lý & Cam kết" badge="06">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-3">
                  <Field
                    label="Thẻ nhãn section"
                    hint="Ví dụ: 05 — Our Approach"
                    value={get("philTag")}
                    onChange={(v) => set("philTag", v)}
                  />
                  <Field
                    label="Tiêu đề section"
                    value={get("philTitle")}
                    onChange={(v) => set("philTitle", v)}
                  />
                </div>
                <Field
                  label="Mô tả section"
                  ...
                  rows={3}
                  value={get("philDesc")}
                  onChange={(v) => set("philDesc", v)}
                />
                <Field
                  label="Quote (chữ nghiêng cuối section)"
                  multiline
                  rows={2}
                  value={get("philQuote")}
                  onChange={(v) => set("philQuote", v)}
                />
                <ListEditor
                  label="Cam kết (3 thẻ)"
              ...
              {/* ══════════════════════ CTA ══════════════════════ */}
              <SectionCard
                title="Partnership CTA — Kêu gọi hành động"
                badge="07"
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-3">
                  <Field
                    label="Thẻ nhãn section"
                    hint="Ví dụ: 06 — Partnership"
                    value={get("ctaTag")}
                    onChange={(v) => set("ctaTag", v)}
                  />
                  <Field
                    label="Tiêu đề section"
                    value={get("ctaTitle")}
                  ...
                  />
                  <Field
                    label="Nút CTA 1 (chính — vàng)"
                    value={get("ctaBtn1Text")}
                    onChange={(v) => set("ctaBtn1Text", v)}
                  />
                  <Field
                    label="Nút CTA 2 (phụ — viền)"
                    value={get("ctaBtn2Text")}
                    onChange={(v) => set("ctaBtn2Text", v)}
                  />
                </div>
                <Field
                  label="Đoạn văn 1"
                  multiline
                  rows={3}
                  value={get("ctaParagraph1")}
                  onChange={(v) => set("ctaParagraph1", v)}
                />
                <Field
                  label="Đoạn văn 2"
                  multiline
                  rows={3}
                  value={get("ctaParagraph2")}
                  onChange={(v) => set("ctaParagraph2", v)}
                />
              </SectionCard>
              ...
              <SectionCard title="Newsletter — Đăng ký nhận tin" badge="08">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 mt-3">
                  <Field
                    label="Tiêu đề section"
                    value={get("newsTitle")}
                    onChange={(v) => set("newsTitle", v)}
                  />
                  <Field
                    label="Văn bản nút đăng ký"
                    value={get("newsBtnText")}
                    onChange={(v) => set("newsBtnText", v)}
                  />
                  <Field
                    label="Placeholder email"
                    value={get("newsPlaceholder")}
                    onChange={(v) => set("newsPlaceholder", v)}
                  />
                  <Field
                    label="Tiêu đề thành công"
                    value={get("newsSuccessTitle")}
                    onChange={(v) => set("newsSuccessTitle", v)}
                  />
                </div>
                <Field
                  label="Mô tả section"
                  multiline
                  rows={3}
                  value={get("newsDescription")}
                  onChange={(v) => set("newsDescription", v)}
                />
                <Field
                  label="Disclaimer dưới form"
                  multiline
                  rows={2}
                  value={get("newsDisclaimer")}
                  onChange={(v) => set("newsDisclaimer", v)}
                />
                <Field
                  label="Nội dung thành công (mô tả)"
                  multiline
                  rows={2}
                  value={get("newsSuccessDesc")}
                  onChange={(v) => set("newsSuccessDesc", v)}
                />
              </SectionCard>
