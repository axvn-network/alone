...
const CHAT_TYPES: {
...
}[] = [
  {
    ...
    label: "WhatsApp",
    color: "#25D366",
    hint: "Nhập số điện thoại (chỉ số, không dấu +), VD: 84987654321",
  },
  {
    type: "telegram",
    label: "Telegram",
    color: "#229ED9",
    hint: "Nhập username (@ten) hoặc số điện thoại, VD: AXVNtech",
  },
  {
    type: "zalo",
    label: "Zalo",
    color: "#0068FF",
    hint: "Nhập số điện thoại Zalo, VD: 0987654321",
  },
  {
    type: "livechat",
    label: "Live Chat",
    color: "#C9A24A",
    hint: "Nhập URL đầy đủ của live chat widget, VD: https://...",
  },
];
...
const DEFAULT_NEWS = {
  ...
  newsDescription:
    "Cập nhật góc nhìn thị trường, phân tích xu hướng đầu tư và tin tức mới nhất từ AXVN Tech Holding – gửi tới bạn khi có giá trị thực sự.",
  newsBtnText: "ĐĂNG KÝ",
  newsDisclaimer:
    "Bằng cách đăng ký, bạn đồng ý nhận các thông tin từ AXVN Tech Holding. Bạn có thể hủy đăng ký bất kỳ lúc nào.",
  newsPlaceholder: "Địa chỉ email của bạn",
  newsSuccessTitle: "Cảm ơn bạn đã đăng ký.",
  newsSuccessDesc:
    "Chúng tôi sẽ gửi tới bạn những góc nhìn chuyên sâu quan trọng nhất.",
};
...
export default function SettingsPage() {
  ...
  function update<K extends keyof SiteSettingsData>(
    key: K,
  ...
  const inputCls =
    "w-full bg-AXVN-deep border border-white/10 text-AXVN-ivory text-sm px-4 py-3 focus:outline-none focus:border-AXVN-gold/50 rounded-lg";
  const labelCls =
    "block text-AXVN-silver text-xs font-medium mb-1.5 tracking-wide";
              ...
              {activeSection === "company" && (
                <div className="bg-AXVN-navy border-t-2 border-t-AXVN-gold/30 p-5 rounded-lg">
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <h2 className="text-sm font-bold text-AXVN-ivory tracking-wide">
                      Thông tin công ty
                    </h2>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2">
                      <label className={labelCls}>
                        Đường Dẫn Nhúng Google Maps
                      </label>
                      <input
                        type="text"
                        value={data.googleMap}
                        onChange={(e) => update("googleMap", e.target.value)}
                        className={inputCls}
                      />
                    </div>
                  </div>
              ...
              {activeSection === "chat" && (
                <div className="bg-AXVN-navy border-t-2 border-t-AXVN-gold/30 p-5 rounded-lg space-y-4">
                  ...
                  <p className="text-AXVN-silver/50 text-xs mb-4">
                    Bật tối đa 3 kênh. Các nút được hiển thị góc phải màn hình
                    theo thứ tự từ dưới lên.
                  </p>

                  {CHAT_TYPES.map((cfg) => {
                    const btn = data.chatButtons.find(
                      (b) => b.type === cfg.type,
                    ) ?? {
                      type: cfg.type,
                      enabled: false,
                    ...
                    const hasMessage =
                      cfg.type === "whatsapp" || cfg.type === "telegram";
                    return (
                      <div
                        key={cfg.type}
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2.5">
                            ...
                            />
                            <span className="text-AXVN-ivory text-sm font-semibold">
                              {cfg.label}
                            </span>
                            {btn.enabled && (
                              <span
                                className="text-[10px] font-medium px-1.5 py-0.5 rounded-full text-AXVN-navy"
                                style={{ backgroundColor: cfg.color }}
                              >
                                Đang bật
                              </span>
                          ...
                          <button
                            onClick={() =>
                              updateChat(cfg.type, "enabled", !btn.enabled)
                            }
                            className="flex items-center gap-1.5 text-xs font-medium transition-colors"
                            style={{
                              color: btn.enabled ? cfg.color : "#8fa0b8",
                            }}
                          >
                            {btn.enabled ? (
                        ...
                        {btn.enabled && (
                          <div className="space-y-3">
                            <div>
                              ...
                              <p className="text-AXVN-silver/35 text-[10px] mb-1.5">
                                {cfg.hint}
                              </p>
                              <input
                                type="text"
                                value={btn.value}
                                onChange={(e) =>
                                  updateChat(cfg.type, "value", e.target.value)
                                }
                                placeholder={
                                  cfg.type === "livechat"
                                    ? "https://..."
                  ...
                  <div className="mt-2 pt-4 border-t border-white/5">
                    <p className="text-AXVN-silver/40 text-[10px] uppercase tracking-widest mb-3">
                      Preview thứ tự hiển thị
                    </p>
                    <div className="flex flex-col-reverse gap-2 items-start">
                      {data.chatButtons
                        ...
                        .map((btn) => {
                          ...
                          return (
                            <div
                              key={btn.type}
                              className="flex items-center gap-2 text-xs text-AXVN-silver"
                            >
                              <div
                              ...
                              <span>{c.label}</span>
                              {btn.value && (
                                <span className="text-AXVN-silver/40 truncate max-w-[160px]">
                                  {btn.value}
                                </span>
                              )}
                            </div>
                      ...
                      {data.chatButtons.filter((b) => b.enabled).length ===
                        0 && (
                        <p className="text-AXVN-silver/30 text-xs italic">
                          Chưa có kênh nào được bật
                        </p>
                      )}
                    </div>
                  </div>
              ...
              {activeSection === "social" && (
                <div className="bg-AXVN-navy border-t-2 border-t-AXVN-gold/30 p-5 rounded-lg">
                  ...
                  <div className="space-y-2.5">
                    {data.socialLinks.map((link, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-2 text-AXVN-silver/30 hover:text-red-400 transition-colors shrink-0 rounded-md"
                        >
                    ...
                    <button
                      onClick={addSocial}
                      className="flex items-center gap-1.5 text-xs text-AXVN-gold hover:text-AXVN-champagne transition-colors mt-3 rounded-md"
                    >
              ...
              {activeSection === "newsletter" && (
                <div className="bg-AXVN-navy border-t-2 border-t-AXVN-gold/30 p-5 rounded-lg space-y-5">
                  ...
                  <p className="text-AXVN-silver/50 text-xs">
                    Chỉnh sửa nội dung hiển thị trong phần đăng ký bản tin ở
                    trang chủ.
                  </p>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewsData((p) => ({
                            ...p,
                            newsPlaceholder: e.target.value,
                          }))
                        }
                        className={inputCls}
                      />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewsData((p) => ({
                              ...p,
                              newsSuccessTitle: e.target.value,
                            }))
                          }
                          className={inputCls}
                        />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewsData((p) => ({
                              ...p,
                              newsSuccessDesc: e.target.value,
                            }))
                          }
                          className={inputCls}
                        />
                  ...
                  <div className="pt-3 border-t border-white/5">
                    ...
                    <div className="bg-AXVN-deep border border-AXVN-gold/10 rounded-lg p-5 text-center">
                      <p className="text-AXVN-ivory text-sm font-bold mb-2">
                        {newsData.newsTitle || "—"}
                      </p>
                      <p className="text-AXVN-silver/60 text-xs mb-3">
                      ...
                      <p className="text-AXVN-silver/25 text-[10px] mt-3">
                        {newsData.newsDisclaimer || "—"}
                      </p>
                    </div>
                  </div>
                </div>
