...
function actionColor(action: string) {
  const k = Object.keys(ACTION_COLORS).find((k) =>
    action.toLowerCase().includes(k),
  );
  return k
    ? ACTION_COLORS[k]
    : "bg-AXVN-deep text-AXVN-silver/60 border-white/10";
}
...
export function AuditLogClient({ logs, total, page, totalPages }: Props) {
  ...
  const inputCls =
    "w-full py-2 px-3 bg-AXVN-navy border border-white/10 text-AXVN-ivory text-xs rounded-lg focus:outline-none focus:border-AXVN-gold/40";
    ...
    <>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-AXVN-silver/50 text-xs">
              {total.toLocaleString("vi-VN")} bản ghi · tự xóa sau 365 ngày
            </p>
          </div>
        </div>
        <button
      ...
      <div className="bg-AXVN-deep border border-AXVN-gold/10 rounded-xl p-4 mb-5 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-AXVN-silver/50 text-xs font-semibold uppercase tracking-wider">
            Bộ lọc
          </p>
          {hasFilters && (
            <button
              onClick={clearFilters}
              className="flex items-center gap-1 text-xs text-AXVN-silver/40 hover:text-red-400 transition-colors"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              push({ collection: e.target.value, page: "1" });
            }}
            className={inputCls}
          >
            {COLLECTIONS.map((c) => (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {logs.map((log) => (
                  <tr
                    key={String(log._id)}
                    className="border-b border-AXVN-gold/5 hover:bg-AXVN-navy/30 transition-colors"
                  >
                    <td className="px-4 py-3 text-AXVN-silver/50 text-xs whitespace-nowrap">
                      {timeAgo(String(log.createdAt))}
                    </td>
                    <td className="px-4 py-3">
                      <p className="text-AXVN-ivory text-xs font-medium">
                        {log.actor?.name || "—"}
                      </p>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-[11px] font-mono font-semibold px-2 py-1 border rounded-lg ${actionColor(log.action)}`}
                      >
                        {log.action}
                      </span>
                    </td>
