...
export default function PublicUsersPage() {
  ...
  async function toggleActive(id: string, current: boolean, name: string) {
    ...
    if (data.success) {
      toast.success(
        `Đã ${!current ? "kích hoạt" : "vô hiệu hoá"} tài khoản "${name}".`,
      );
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`Xóa vĩnh viễn tài khoản "${name}"? Không thể hoàn tác.`))
      return;
            ...
            <div className="flex items-center gap-3 flex-1">
              ...
              <div>
                ...
                <p className="text-AXVN-silver/40 text-xs">
                  {meta.total.toLocaleString("vi-VN")} tài khoản đã đăng ký
                </p>
              </div>
            </div>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              onClick={() =>
                                toggleActive(u._id, u.isActive, u.name)
                              }
          ...
          {meta.pages > 1 && (
            <div className="flex items-center justify-between mt-4 text-xs text-AXVN-silver/40">
              <p>
                ...
                {meta.total.toLocaleString("vi-VN")} người dùng
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    setPage((p) => Math.max(1, p - 1));
                    void fetchUsers(Math.max(1, page - 1));
                  }}
                  disabled={page <= 1}
                ...
                <button
                  onClick={() => {
                    ...
                    void fetchUsers(Math.min(meta.pages, page + 1));
                  }}
