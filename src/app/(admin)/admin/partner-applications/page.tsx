...
import {
  ...
  APPLICATION_STATUS_CLS as STATUS_CLS,
  ADMIN_PAGE_CLS,
...
export default function PartnerApplicationsPage() {
  ...
  const scoreAvg = (s: Application["assessmentScore"]) =>
    Math.round(
      (s.technical + s.financial + s.legal + s.strategic + s.network) / 5,
    );
  ...
  return (
    <div className={ADMIN_PAGE_CLS}>
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden">
        ...
        <main className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <button
              onClick={load}
              className="p-2 border border-AXVN-gold/20 rounded-lg hover:border-AXVN-gold/40 transition-colors"
            >
              <RefreshCw
                className={`w-4 h-4 text-AXVN-silver ${loading ? "animate-spin" : ""}`}
              />
            </button>
          </div>
              ...
              <tbody className="divide-y divide-AXVN-gold/5">
                {loading ? (
                  <tr>
                    <td
                      colSpan={6}
                      className="py-12 text-center text-AXVN-silver/40"
                    >
                      Đang tải...
                    </td>
                  </tr>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span className="text-xs">
                          {ROLE_LABELS[app.desiredRole] || app.desiredRole}
                        </span>
                      </td>
                      <td className="px-4 py-3 hidden lg:table-cell">
                        <span className="font-bold text-AXVN-gold">
                          {scoreAvg(app.assessmentScore)}%
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs border ${STATUS_CLS[app.status] || ""}`}
                        >
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-4">
              <button
                ...
                disabled={page <= 1}
                className="p-2 border border-AXVN-gold/20 rounded-lg disabled:opacity-30"
              >
                <ChevronLeft className="w-4 h-4 text-AXVN-silver" />
              </button>
              <span className="text-AXVN-silver/60 text-sm">
      ...
      {selected && (
        ...
        >
          <div
            className="w-full max-w-2xl bg-AXVN-deep border border-AXVN-gold/20 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
          ...
          >
            <div className="sticky top-0 flex items-center justify-between px-6 py-4 border-b border-AXVN-gold/10 bg-AXVN-deep z-10">
              <div>
                ...
                <p className="text-AXVN-silver/60 text-xs">
                  {selected.email} · {ROLE_LABELS[selected.desiredRole]}
                </p>
              </div>
              <button
                onClick={() => setSelected(null)}
                className="p-1.5 border border-AXVN-gold/20 rounded-lg hover:border-AXVN-gold/50"
              >
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-5 gap-3">
                {Object.entries(selected.assessmentScore).map(([k, v]) => (
                  <div
                    key={k}
                    className="text-center p-3 bg-white/[0.03] rounded-xl border border-AXVN-gold/10"
                  >
                    <p className="text-AXVN-gold font-bold text-xl">{v}</p>
                    <p className="text-AXVN-silver/50 text-xs mt-1 capitalize">
                      {k}
                    </p>
                  </div>
                ))}
              ...
              <div className="space-y-2 text-sm">
                {selected.company && (
                  <p className="text-AXVN-silver/70">
                    <span className="text-AXVN-silver/40">Tổ chức:</span>{" "}
                    {selected.company}{" "}
                    {selected.position && `— ${selected.position}`}
                  </p>
                )}
                {selected.capitalRange && (
                  <p className="text-AXVN-silver/70">
                    <span className="text-AXVN-silver/40">Vốn:</span>{" "}
                    {selected.capitalRange}
                  </p>
                )}
              </div>
              ...
              {/* Notes */}
              <div>
                <label className="block text-xs text-AXVN-silver/50 mb-1">
                  Ghi chú admin
                </label>
                <textarea
                  value={notesDraft}
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {["under_review", "shortlisted", "approved", "rejected"].map(
                  (s) => (
                    <button
                      key={s}
              ...
              <button
                ...
                onClick={() =>
                  updateStatus(selected._id, selected.status, notesDraft)
                }
