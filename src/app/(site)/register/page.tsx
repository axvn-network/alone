...
import { Eye, EyeOff, Lock, Mail, Loader2, User, Phone, Newspaper } from "lucide-react";
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (!form.email.trim())        { setError("Vui lòng nhập địa chỉ email."); return; }
    ...
    try {
      ...
      if (!data.success) { setError(data.message ?? "Đăng ký thất bại. Vui lòng thử lại."); return; }
      router.push("/");
      router.refresh();
    } catch {
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex items-center justify-center gap-2 mt-4">
            ...
            <span className="text-AXVN-gold text-xs font-semibold tracking-widest uppercase">Tạo Tài Khoản</span>
          </div>
        </div>

        {/* Card */}
        <div className="bg-[#06101a] border border-AXVN-gold/15 rounded-2xl p-8 shadow-2xl">
          <h1 className="text-AXVN-ivory font-semibold text-xl mb-1">Đăng ký tài khoản</h1>
          <p className="text-AXVN-silver/50 text-sm mb-7">
            Đã có tài khoản?{" "}
            <Link href="/login" className="text-AXVN-gold hover:text-AXVN-champagne transition-colors font-medium">
              Đăng nhập
            </Link>
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required placeholder="Nguyễn Văn A" autoComplete="name"
            ...
            <div>
              <label className="block text-AXVN-silver/70 text-xs font-medium mb-1.5 tracking-wide">
                Điện thoại <span className="text-AXVN-silver/30 font-normal">(tùy chọn)</span>
              </label>
              <div className="relative">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  onChange={handleChange} required placeholder="Ít nhất 8 ký tự" autoComplete="new-password"
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-AXVN-silver/30" />
                <input
                  name="confirmPassword" type={showCpw ? "text" : "password"} value={form.confirmPassword}
                  onChange={handleChange} required placeholder="Nhập lại mật khẩu" autoComplete="new-password"
                  className="w-full bg-AXVN-deep border border-white/10 text-AXVN-ivory text-sm pl-10 pr-11 py-3 focus:outline-none focus:border-AXVN-gold/50 transition-colors rounded-xl"
                />
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              <input
                name="newsletterSubscribed" type="checkbox"
                checked={form.newsletterSubscribed} onChange={handleChange}
            ...
            <p className="text-AXVN-silver/25 text-[11px] text-center leading-relaxed">
              ...
              <Link href="/terms-of-use" className="text-AXVN-gold/50 hover:text-AXVN-gold transition-colors">Điều khoản sử dụng</Link>
              {" "}và{" "}
              <Link href="/privacy-policy" className="text-AXVN-gold/50 hover:text-AXVN-gold transition-colors">Chính sách bảo mật</Link>.
            </p>
          </form>
        </div>

        <p className="text-center text-AXVN-silver/20 text-[11px] mt-6">
          <Link href="/" className="hover:text-AXVN-silver/40 transition-colors">← Quay về trang chủ</Link>
        </p>
      </div>
    </div>
  );
