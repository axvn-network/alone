...
const CAT_COLORS: Record<string, string> = {
  ...
  "Thâu Tóm Doanh Nghiệp (M&A)": "bg-blue-500/10 text-blue-400 border-blue-500/20",
  "Đầu Tư Tư Nhân (Private Equity)": "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
  "Công Nghệ AI": "bg-purple-500/10 text-purple-400 border-purple-500/20",
  "Tài Sản Số & Blockchain": "bg-orange-500/10 text-orange-400 border-orange-500/20",
  ...
  "Góc Nhìn Thị Trường": "bg-indigo-500/10 text-indigo-400 border-indigo-500/20",
  "Tin Tức Tập Đoàn": "bg-red-500/10 text-red-400 border-red-500/20",
  "Quản Lý Đầu Tư Chiến Lược": "bg-teal-500/10 text-teal-400 border-teal-500/20",
};
...
const formatDate = (dateString?: string) => {
  ...
  try {
    ...
    return new Intl.DateTimeFormat("vi-VN", { year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }).format(d);
  } catch {
    return dateString;
  }
};
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
            {[
              ...
              <motion.div key={b.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12, duration: 0.5, ease: "easeOut" }} className="bg-[#0d1e30] border border-[#C9A24A]/15 p-5 text-left hover:border-[#C9A24A]/40 transition-colors rounded-xl">
                ...
                <p className="text-AXVN-ivory text-sm font-semibold mb-1">{b.title}</p>
                <p className="text-AXVN-silver/60 text-xs">{b.desc}</p>
              </motion.div>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/invest-with-axvn" className="px-8 py-4 bg-[#C9A24A] hover:bg-[#E6C879] text-[#07111D] font-bold text-sm transition-all shadow-xl hover:-translate-y-0.5 rounded-lg">Tìm Hiểu Hành Trình</Link>
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[80vw] max-w-[300px] sm:max-w-[320px] bg-[#03080e] overflow-y-auto shadow-2xl border-r border-gray-200">
            <div className="flex items-center justify-between p-4 border-b border-white/10 bg-[#07111D] sticky top-0 z-10">
              <h2 className="font-bold text-AXVN-ivory flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setMobileDrawerOpen(false)} className="p-1.5 hover:bg-white/5 text-AXVN-silver/60 hover:text-AXVN-ivory transition-colors "><X className="w-5 h-5" /></button>
              </div>
            </div>
