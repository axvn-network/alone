...
const CAT_ICONS: Record<string, React.ReactNode> = {
  "Bất Động Sản": <Building2 className="w-3.5 h-3.5" />,
  "Thâu Tóm Doanh Nghiệp (M&A)": <Briefcase className="w-3.5 h-3.5" />,
  "Đầu Tư Tư Nhân (Private Equity)": <TrendingUp className="w-3.5 h-3.5" />,
  "Công Nghệ AI": <Cpu className="w-3.5 h-3.5" />,
  "Tài Sản Số & Blockchain": <Cpu className="w-3.5 h-3.5" />,
  "Khách Sạn & Nghỉ Dưỡng": <UtensilsCrossed className="w-3.5 h-3.5" />,
  "Thương Mại & Phân Phối": <Truck className="w-3.5 h-3.5" />,
  "Góc Nhìn Thị Trường": <LineChart className="w-3.5 h-3.5" />,
  "Tin Tức Tập Đoàn": <Newspaper className="w-3.5 h-3.5" />,
  "Quản Lý Đầu Tư Chiến Lược": <Handshake className="w-3.5 h-3.5" />,
};
const CAT_COLORS: Record<string, string> = {
  "Bất Động Sản": "bg-amber-500/10 text-amber-400 border-amber-500/20",
  "Thâu Tóm Doanh Nghiệp (M&A)":
    "bg-blue-500/10 text-blue-400 border-blue-500/20",
  "Đầu Tư Tư Nhân (Private Equity)":
    "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
  "Công Nghệ AI": "bg-purple-500/10 text-purple-400 border-purple-500/20",
  "Tài Sản Số & Blockchain":
    "bg-orange-500/10 text-orange-400 border-orange-500/20",
  "Khách Sạn & Nghỉ Dưỡng": "bg-pink-500/10 text-pink-400 border-pink-500/20",
  "Thương Mại & Phân Phối": "bg-cyan-500/10 text-cyan-400 border-cyan-500/20",
  "Góc Nhìn Thị Trường":
    "bg-indigo-500/10 text-indigo-400 border-indigo-500/20",
  "Tin Tức Tập Đoàn": "bg-red-500/10 text-red-400 border-red-500/20",
...
const CAT_IMAGES: Record<string, string> = {
  ...
  "Thâu Tóm Doanh Nghiệp (M&A)": "/black-men-cafe-have-business.jpg",
  "Đầu Tư Tư Nhân (Private Equity)": "/plants-coins.jpg",
  "Công Nghệ AI": "/businessman-reading.jpg",
  "Tài Sản Số & Blockchain": "/gold-coins.jpg",
  "Khách Sạn & Nghỉ Dưỡng": "/employees.jpg",
  "Thương Mại & Phân Phối": "/discussing-business.jpg",
  "Góc Nhìn Thị Trường": "/strategy-ideas.jpg",
  "Tin Tức Tập Đoàn": "/african-man-black-suit.jpg",
  "Quản Lý Đầu Tư Chiến Lược": "/portrait-smiling.jpg",
};
...
export default function InsightsClient({
  ...
  const activeFilterCount =
    selectedCategories.length + selectedTags.length + (search ? 1 : 0);
  ...
  const Sidebar = () => (
    <aside className="flex flex-col gap-4">
            ...
            >
              <span className="flex items-center gap-2">
                ...
                <span className="truncate">
                  {cat.label === "Digital Assets & Blockchain"
                    ? "Digital & Blockchain"
                    : cat.label === "Strategic Investment Management"
                      ? "Strategic Mgmt"
                      : cat.label}
                </span>
              </span>
              <span className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 shrink-0 ">
                {catCounts[cat.label] || 0}
              </span>
            </button>
          ))}
        </div>
      ...
      {allTags.length > 0 && (
        <div className="bg-white border border-gray-200 shadow-sm p-4 ">
          <h3 className="text-[10px] font-bold text-gray-500 tracking-widest uppercase mb-3">
            # Tags
          </h3>
      ...
      <article className="group bg-white border border-gray-200 shadow-sm  overflow-hidden hover:border-[#C9A24A]/30 hover:shadow-2xl hover:shadow-black/30 transition-all duration-300 flex flex-col">
        ...
        <div className="relative h-48 sm:h-52 overflow-hidden shrink-0">
          ...
          <div className="absolute top-3 left-3">
            <span
              className={`text-[10px] font-semibold px-2.5 py-1 border  ${CAT_COLORS[article.category] || "bg-gray-50 text-gray-600 border-gray-200"}`}
            >
              {article.category}
            </span>
          </div>
        ...
        <div className="p-4 sm:p-5 flex flex-col flex-1">
          ...
          <div className="flex flex-wrap gap-1 mb-4">
            {(article.tags || []).slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-[10px] px-2 py-0.5 bg-gray-50 border border-gray-200 text-gray-400 "
              >
                {tag}
              </span>
            ))}
    ...
    return (
      <article className="group bg-white border border-gray-200 shadow-sm  overflow-hidden hover:border-[#C9A24A]/30 hover:shadow-2xl hover:shadow-black/30 transition-all duration-300 flex flex-col sm:flex-row">
        ...
        <div className="p-4 sm:p-5 flex flex-col flex-1 min-w-0">
          ...
          <h3 className="font-bold text-gray-900 text-sm sm:text-base leading-snug mb-1.5 group-hover:text-[#C9A24A] transition-colors line-clamp-2">
            {article.title}
          </h3>
          ...
          <div className="flex items-center justify-between mt-auto gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <Link
                href={`/insights/${article.slug}`}
                className="px-3 py-1.5 bg-[#C9A24A]/10 text-[#C9A24A] border border-[#C9A24A]/20 text-[11px] font-semibold  hover:bg-[#C9A24A] hover:text-[#07111D] transition-all flex items-center gap-1"
              >
                Đọc <ArrowRight className="w-3 h-3" />
              </Link>
            </div>
  ...
  return (
    <>
      ...
      <motion.section
        {...fadeUp}
        className="relative bg-[#07111D] overflow-hidden"
        style={{
          paddingTop: "calc(var(--section-py) + 4rem)",
      ...
      >
        ...
        <div className="relative max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12 text-center">
          <div className="flex items-center justify-center gap-3 mb-5">
            ...
            <span className="text-[#C9A24A] text-[10px] font-semibold tracking-[0.22em] uppercase">
          ...
          >
            Góc Nhìn &amp;{" "}
            <span className="font-bold bg-gradient-to-r from-[#C9A24A] to-[#E6C879] bg-clip-text text-transparent">
              Chuyên Sâu
            </span>
          </h1>
          <p
            className="text-gray-400 max-w-2xl mx-auto mb-8 sm:mb-10 leading-[1.8] px-2"
            style={{ fontSize: "clamp(0.9rem, 1.5vw + 0.3rem, 1.125rem)" }}
          >
            Góc nhìn đầu tư, phân tích thị trường và bình luận chiến lược từ
            AXVN Tech Holding.
          </p>
          {/* Search bar */}
          <div className="max-w-2xl mx-auto mb-6 sm:mb-8 relative">
            ...
            {search && (
              <button
                onClick={() => setSearch("")}
                className="absolute right-4 sm:right-5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white"
              >
                <X className="w-4 h-4" />
              </button>
            )}
      ...
      <div className="w-full px-3 sm:px-4 md:pl-0 md:pr-8 lg:pr-12 py-6 sm:py-10">
        <div className="flex gap-6 lg:gap-8 items-start">
          ...
          <motion.div
            {...fadeLeft}
            className="hidden md:block md:w-56 lg:w-72 xl:w-80 shrink-0 sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto pb-4"
          >
            <Sidebar />
          </motion.div>
          ...
          <div ref={resultsRef} className="flex-1 min-w-0 w-full">
            ...
            <div className="md:hidden flex items-center gap-2 mb-4">
              ...
              >
                ...
                {activeFilterCount > 0 && (
                  <span className="bg-[#C9A24A] text-white text-[10px] font-bold w-4 h-4 flex items-center justify-center ">
                    {activeFilterCount}
                  </span>
                )}
              </button>
              <div className="flex-1 relative min-w-0">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 bg-white border border-white/10 text-sm text-gray-900 placeholder:text-gray-500 focus:outline-none focus:border-[#C9A24A]/60 transition-all "
                />
              </div>
            </div>
              ...
              <motion.section {...fadeUp} className="mb-8 sm:mb-10">
                <div className="flex items-center gap-3 mb-4">
                  <span className="text-[10px] font-bold text-gray-500 tracking-widest uppercase flex items-center gap-1.5 shrink-0">
                    <BookOpen className="w-3.5 h-3.5 text-[#C9A24A]" /> Bài Viết
                    Mới Nhất
                  </span>
                ...
                <article className="group bg-white border border-gray-200 shadow-sm  overflow-hidden hover:border-[#C9A24A]/30 hover:shadow-2xl hover:shadow-black/30 transition-all duration-300">
                  <div className="flex flex-col md:flex-row">
                    <div className="md:w-[48%] relative h-56 sm:h-64 md:h-auto md:min-h-[320px] shrink-0 overflow-hidden">
                      <Image
                        src={
                          ...
                          CAT_IMAGES[filtered[0].category] ||
                          "/business.jpg"
                        }
                      ...
                      <div className="absolute top-3 left-3 flex items-center gap-2 flex-wrap">
                        <span
                          className={`text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1 sm:py-1.5 border  ${CAT_COLORS[filtered[0].category] || "bg-gray-50 text-gray-600 border-gray-200"}`}
                        >
                          {filtered[0].category}
                        </span>
                      </div>
                    ...
                    <div className="p-5 sm:p-8 md:p-10 flex flex-col justify-center flex-1">
                      ...
                      <p aria-hidden="true" className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-2 sm:mb-3 leading-snug group-hover:text-[#C9A24A] transition-colors">{filtered[0].title}</p>
                      <p className="text-gray-500 text-sm sm:text-base leading-relaxed mb-4 sm:mb-5 line-clamp-3 md:line-clamp-none">
                        {filtered[0].excerpt}
                      </p>
                      <div className="flex flex-wrap gap-1.5 mb-5 sm:mb-6">
                        {(filtered[0].tags || []).map((tag) => (
                          <span
                            key={tag}
                            className="text-xs px-2.5 py-1 bg-gray-50 border border-gray-200 text-gray-400 "
                          >
                            {tag}
                          </span>
                        ))}
            ...
            <motion.div
              {...stagger}
              className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3 mb-4 sm:mb-5 py-3 border-t border-b border-gray-200"
            >
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {activeFilterCount > 0 && (
                  <button
                    onClick={clearAll}
                    className="flex items-center gap-1 text-xs text-red-400 font-medium bg-red-500/10 px-2.5 py-1.5 border border-red-500/20 "
                  >
                ...
                {selectedCategories.slice(0, 2).map((c) => (
                  <span
                    key={c}
                    className="flex items-center gap-1 text-[10px] bg-[#C9A24A]/10 text-[#C9A24A] border border-[#C9A24A]/20 px-2 py-1 font-medium max-w-[100px] truncate "
                  >
                    {c}
                    <button
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto">
                <div className="flex bg-gray-50 border border-gray-200 overflow-hidden ">
                  <button
                    ...
                    className={`p-2.5 transition-colors ${viewMode === "grid" ? "bg-[#C9A24A]/20 text-[#C9A24A]" : "text-gray-500 hover:text-gray-900"}`}
                  ...
                  <button
                    ...
                    className={`p-2.5 transition-colors ${viewMode === "list" ? "bg-[#C9A24A]/20 text-[#C9A24A]" : "text-gray-500 hover:text-gray-900"}`}
            ...
            {paginated.length === 0 ? (
              ...
              >
                ...
                <p className="text-gray-500 text-sm mb-5 sm:mb-6 max-w-xs">
                  Thử điều chỉnh từ khóa hoặc bỏ bớt bộ lọc.
                </p>
                <button
                  onClick={clearAll}
                  className="px-5 sm:px-6 py-2.5 sm:py-3 bg-[#C9A24A]/10 text-[#C9A24A] border border-[#C9A24A]/20 text-sm font-semibold  hover:bg-[#C9A24A] hover:text-[#07111D] transition-all"
                >
                  Xóa Tất Cả Bộ Lọc
                </button>
              </motion.div>
            ...
            {totalPages > 1 && (
              <motion.div
                {...stagger}
                className="flex items-center justify-center gap-1.5 sm:gap-2 mt-8 sm:mt-10 flex-wrap"
              >
                <button
                  onClick={() => {
                  ...
                  disabled={page === 1}
                  className="p-2.5 border border-white/10 text-gray-500 hover:border-[#C9A24A]/50 hover:text-[#C9A24A] disabled:opacity-30 disabled:cursor-not-allowed transition-all bg-white "
                >
      ...
      <motion.section
        {...fadeUp}
        className="bg-[#07111D] section-mx section-my overflow-hidden relative rounded-2xl"
        style={{
      ...
      >
        ...
        <div className="relative max-w-[900px] mx-auto px-6 text-center">
          ...
          <h2
            ...
            style={{
              fontSize: "clamp(1.5rem, 3.2vw + 0.4rem, 3.25rem)",
              letterSpacing: "0.04em",
            }}
          >
            Mở Khóa Thông Tin
          ...
          <p
            className="text-gray-400 mb-10 max-w-xl mx-auto leading-[1.8]"
            style={{ fontSize: "clamp(0.9rem, 1.5vw + 0.3rem, 1.125rem)" }}
          >
            Tiếp cận không giới hạn các nghiên cứu cao cấp, tài liệu chiến lược
            và nguồn tư vấn chuyên gia được tuyển chọn dành cho nhà đầu tư
            nghiêm túc.
          </p>
            ...
            {[
              {
                ...
                title: "Bài Viết Cao Cấp",
                desc: "Phân tích chuyên sâu & nghiên cứu",
              },
              {
                icon: <Users className="w-5 h-5" />,
                title: "Tư Vấn Chuyên Gia",
                desc: "Kết nối trực tiếp với đội ngũ",
              },
              {
                icon: <Star className="w-5 h-5" />,
                title: "Báo Cáo Thị Trường",
                desc: "Góc nhìn ngành độc quyền",
              },
            ].map((b, i) => (
              <motion.div
                key={b.title}
                ...
                className="bg-[#0d1e30] border border-[#C9A24A]/15 p-5 text-left hover:border-[#C9A24A]/40 transition-colors rounded-xl"
              >
                <div className="text-[#C9A24A] mb-3">{b.icon}</div>
                <p className="text-AXVN-ivory text-sm font-semibold mb-1">
                  {b.title}
                </p>
                <p className="text-AXVN-silver/60 text-xs">{b.desc}</p>
              </motion.div>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              ...
              className="px-8 py-4 bg-[#C9A24A] hover:bg-[#E6C879] text-[#07111D] font-bold text-sm transition-all shadow-xl hover:-translate-y-0.5 rounded-lg"
            >
              Tìm Hiểu Hành Trình
            </Link>
            <Link
              href="/contact"
              className="px-8 py-4 border border-white/20 hover:border-white/40 text-white font-bold text-sm transition-all hover:bg-white/5 rounded-lg"
            >
              Liên Hệ Đội Ngũ
            </Link>
          </div>
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[80vw] max-w-[300px] sm:max-w-[320px] bg-[#03080e] overflow-y-auto shadow-2xl border-r border-gray-200">
            <div className="flex items-center justify-between p-4 border-b border-white/10 bg-[#07111D] sticky top-0 z-10">
              <p aria-hidden="true" className="font-bold text-AXVN-ivory flex items-center gap-2"><Filter className="w-4 h-4 text-[#C9A24A]" /> Bộ Lọc
                {activeFilterCount > 0 && (
                  <span className="bg-[#C9A24A] text-[#07111D] text-[10px] font-bold px-2 py-0.5 ">
                    {activeFilterCount}
                  </span>
                )}</p>
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => setMobileDrawerOpen(false)}
                  className="p-1.5 hover:bg-white/5 text-AXVN-silver/60 hover:text-AXVN-ivory transition-colors "
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
