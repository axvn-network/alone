...
const TIER_COLORS: Record<string, string> = {
  ...
  expansion:   "from-gvi-gold/20 to-gvi-gold/5 border-gvi-gold/40",
  ...
  institution: "from-gvi-gold/20 to-gvi-gold/5 border-gvi-gold/40",
...
export default function InvestmentPlansClient() {
  ...
  if (plans.length === 0) return (
    <div className="text-center py-24 text-gvi-silver/50 text-sm">
  ...
  return (
    <>
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-5">
        {plans.map((plan) => {
          const accent   = TIER_ACCENT[plan.tier]   || "text-gvi-gold";
          const colors   = TIER_COLORS[plan.tier]   || "from-gvi-gold/10 to-gvi-gold/5 border-gvi-gold/20";
          const iconBg   = TIER_ICON_BG[plan.tier]  || "bg-gvi-gold/15";
          ...
          return (
            <div
              ...
              className={`relative cursor-pointer group flex flex-col rounded-xl border bg-gradient-to-b ${colors} p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-black/30 ${isActive ? "ring-2 ring-gvi-gold/60" : ""}`}
            >
              {/* Badge */}
              {plan.badge && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-gvi-gold text-gvi-navy text-[10px] font-black tracking-[0.15em] uppercase rounded-full shadow-md whitespace-nowrap">
                  {plan.badge}
                </span>
              )}
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-gvi-silver/40 text-[10px] font-mono uppercase tracking-widest">{plan.nameEn}</p>
                </div>
              </div>

              {/* Shareholder type */}
              <p className="text-gvi-silver/50 text-[10px] uppercase tracking-widest mb-3 font-semibold">{plan.shareholderType}</p>

              {/* Tagline */}
              <p className="text-gvi-ivory/80 text-xs leading-relaxed mb-5 flex-1">{plan.tagline}</p>

              {/* Commitment */}
              <div className="pt-4 border-t border-white/10 mb-4">
                <p className="text-gvi-silver/40 text-[10px] uppercase tracking-widest mb-1">Vốn góp tối thiểu</p>
                <p className={`font-black text-base ${accent}`}>{formatVND(plan.minCommitment)}</p>
                {plan.maxCommitment > 0 && (
                  <p className="text-gvi-silver/30 text-[10px] mt-0.5">Tối đa: {formatVND(plan.maxCommitment)}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-5">
                <div className="bg-black/20 rounded-lg p-2.5">
                  ...
                  <p className="text-gvi-ivory text-[11px] font-semibold leading-tight">{plan.equityRange}</p>
                </div>
                <div className="bg-black/20 rounded-lg p-2.5">
                  <p className="text-gvi-silver/40 text-[9px] uppercase tracking-wider mb-0.5">Thời Hạn</p>
                  <p className="text-gvi-ivory text-[11px] font-semibold leading-tight">{plan.duration}</p>
                </div>
              </div>
              ...
              <ul className="space-y-1.5 mb-5">
                {(plan.rights || plan.benefits || []).slice(0, 2).map((r, i) => (
                  <li key={i} className="flex items-start gap-1.5">
                    ...
                    <span className="text-gvi-silver/65 text-[11px] leading-relaxed">{r}</span>
                  </li>
                ))}
                {(plan.rights || plan.benefits || []).length > 2 && (
                  <li className="text-[11px] text-gvi-silver/35 pl-4.5">
              ...
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 py-2.5 text-[11px] font-bold tracking-[0.1em] uppercase transition-all rounded-lg border ${
                  plan.highlighted
                    ? "bg-gvi-gold text-gvi-navy border-gvi-gold hover:bg-gvi-champagne"
      ...
      {selected && (
        ...
        >
          <div
            className="relative w-full max-w-2xl max-h-[92vh] overflow-y-auto bg-[#06101a] border border-gvi-gold/20 rounded-2xl shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Modal header */}
            <div className="sticky top-0 z-10 flex items-center justify-between p-5 border-b border-gvi-gold/10 bg-[#06101a]">
              <div className="flex items-center gap-3">
                <div className={`w-9 h-9 rounded-lg ${TIER_ICON_BG[selected.tier] || "bg-gvi-gold/15"} flex items-center justify-center shrink-0`}>
                  <TierIcon tier={selected.tier} className={`w-4 h-4 ${TIER_ACCENT[selected.tier] || "text-gvi-gold"}`} />
                </div>
                <div>
                  <h2 className={`font-black text-lg ${TIER_ACCENT[selected.tier] || "text-gvi-gold"}`}>{selected.name}</h2>
                  <p className="text-gvi-silver/40 text-[10px] font-mono">{selected.shareholderType}</p>
                </div>
              </div>
              <button
                onClick={() => setSelected(null)}
                className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-gvi-silver hover:text-white transition-colors shrink-0"
                aria-label="Đóng"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={kv.label} className="bg-black/30 rounded-xl p-3 text-center">
                    <p className="text-gvi-silver/40 text-[9px] uppercase tracking-widest mb-1">{kv.label}</p>
                    <p className={`font-black text-xs ${TIER_ACCENT[selected.tier] || "text-gvi-gold"}`}>{kv.value}</p>
                  </div>
                ))}
              </div>

              {/* Tagline */}
              <p className="text-gvi-ivory/80 text-sm leading-relaxed border-b border-gvi-gold/10 pb-4">{selected.tagline}</p>
              ...
              {tab === "rights" && (
                <div>
                  <p className="text-gvi-silver/50 text-[10px] uppercase tracking-widest mb-3 font-semibold">Quyền của cổ đông khi tham gia</p>
                  <ul className="space-y-2.5">
                    {(selected.rights || selected.benefits || []).map((r, i) => (
                      <li key={i} className="flex items-start gap-2.5 p-3 bg-gvi-deep/60 rounded-lg">
                        <CheckCircle className={`w-4 h-4 mt-0.5 shrink-0 ${TIER_ACCENT[selected.tier] || "text-gvi-gold"}`} />
              ...
              {tab === "obligations" && (
                <div>
                  <p className="text-gvi-silver/50 text-[10px] uppercase tracking-widest mb-3 font-semibold">Nghĩa vụ bắt buộc theo pháp luật &amp; nội quy công ty</p>
                  <ul className="space-y-2.5">
              ...
              {tab === "documents" && (
                <div>
                  <p className="text-gvi-silver/50 text-[10px] uppercase tracking-widest mb-3 font-semibold">Hồ sơ cần chuẩn bị theo NQ 05/2025/NQ-CP</p>
                  ...
                  <p className="text-gvi-silver/30 text-[10px] mt-3 italic">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-2">
                <Link
                  href="/invest-with-gvi#enquiry"
                  className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-gvi-gold to-gvi-champagne text-gvi-navy font-bold text-sm uppercase tracking-wider rounded-xl hover:opacity-90 transition-opacity"
                >
                  Đăng Ký Góp Vốn
                </Link>
                <Link
                  href="/contact"
                  className="flex-1 flex items-center justify-center gap-2 py-3.5 border border-gvi-gold/25 text-gvi-gold font-semibold text-sm uppercase tracking-wider rounded-xl hover:bg-gvi-gold/5 transition-colors"
                >
                  Hỏi Thêm
                </Link>
              </div>
