...
import AiAssistPanel, { DOC_AI_ACTIONS } from "@/components/AiAssistPanel";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#07111D] border border-red-500/20 rounded-2xl shadow-2xl w-full max-w-md p-6">
        <button onClick={onCancel} className="absolute top-4 right-4 p-1.5 text-fortress-silver/40 hover:text-fortress-ivory transition-colors rounded-lg hover:bg-white/5">
          <X className="w-4 h-4" />
        </button>
        <div className="w-12 h-12 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center justify-center mb-4">
          <AlertTriangle className="w-6 h-6 text-red-400" />
        </div>
        <h2 className="text-lg font-bold text-fortress-ivory mb-1">Xóa tài liệu?</h2>
        <p className="text-fortress-silver/50 text-sm mb-5 leading-relaxed">
          Xóa vĩnh viễn <span className="text-fortress-ivory font-semibold">&ldquo;{doc.title}&rdquo;</span>. Không thể hoàn tác.
        </p>
        <div className="flex gap-2.5">
          <button onClick={onCancel} disabled={loading} className="flex-1 py-2.5 border border-white/10 text-fortress-silver/70 text-sm font-semibold hover:border-white/20 hover:text-fortress-ivory transition-all rounded-xl">
            Hủy
          </button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto">
      ...
      <div className="relative bg-[#07111D] border border-white/10 rounded-2xl shadow-2xl w-full max-w-lg p-6 my-8">
        ...
        <h2 className="text-lg font-bold text-fortress-ivory mb-5">
        ...
        <div className="space-y-4">
          {/* Title */}
          <div>
            <label className="text-xs font-semibold text-fortress-silver/60 uppercase tracking-wide mb-1.5 block">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-fortress-ivory placeholder:text-fortress-silver/30 focus:outline-none focus:border-fortress-gold/40"
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-fortress-ivory focus:outline-none focus:border-fortress-gold/40"
            >
              {CATEGORIES.map(([k, v]) => (
            ...
            <div className="flex rounded-xl overflow-hidden border border-white/10 mb-3">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 text-xs font-semibold transition-colors ${
                  fileTab === "upload"
                    ? "bg-fortress-gold/20 text-fortress-gold border-r border-fortress-gold/20"
                    : "text-fortress-silver/50 hover:text-fortress-silver border-r border-white/10"
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 text-xs font-semibold transition-colors ${
                  fileTab === "url"
                    ? "bg-fortress-gold/20 text-fortress-gold"
                    : "text-fortress-silver/50 hover:text-fortress-silver"
                }`}
              >
              ...
              <div>
                ...
                <div
                  ...
                  className={`relative flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl p-6 cursor-pointer transition-colors ${
                    dragOver
                      ? "border-fortress-gold bg-fortress-gold/5"
                      : uploading
                        ? "border-white/10 bg-white/3 cursor-not-allowed"
                        : form.fileUrl && uploadedFileName
                          ? "border-green-500/40 bg-green-500/5"
                          : "border-white/15 hover:border-fortress-gold/40 hover:bg-white/3"
                  }`}
                >
                  {uploading ? (
                    <div className="flex flex-col items-center gap-2 w-full">
                      <Loader2 className="w-8 h-8 text-fortress-gold animate-spin" />
                      <p className="text-xs text-fortress-silver/70">Đang tải lên…</p>
                      <div className="w-full bg-white/10 rounded-full h-1.5 mt-1">
                        <div
                          className="bg-fortress-gold h-1.5 rounded-full transition-all duration-300"
                          style={{ width: `${uploadProgress}%` }}
                        />
                      </div>
                      <p className="text-[10px] text-fortress-silver/50">{Math.round(uploadProgress)}%</p>
                    </div>
                  ) : form.fileUrl && uploadedFileName ? (
                    <div className="flex flex-col items-center gap-1.5">
                      <CheckCircle2 className="w-8 h-8 text-green-400" />
                      <p className="text-sm font-semibold text-green-400">Tải lên thành công</p>
                      <p className="text-xs text-fortress-silver/60 text-center break-all max-w-xs">{uploadedFileName}</p>
                      <button
                        type="button"
                    ...
                    <>
                      <Upload className="w-8 h-8 text-fortress-silver/40" />
                      <p className="text-sm text-fortress-silver/70 font-medium text-center">
                        Kéo thả file vào đây<br />
                        <span className="text-fortress-silver/40">hoặc nhấn để chọn file</span>
                      </p>
                      <p className="text-[10px] text-fortress-silver/30 text-center">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-fortress-ivory focus:outline-none focus:border-fortress-gold/40"
              >
                {FILE_TYPES.map((ft) => (
          ...
          <label className="flex items-center gap-2.5 cursor-pointer">
            <input
              ...
              className="w-4 h-4 accent-fortress-gold"
            />
            <span className="text-sm text-fortress-silver/70">⭐ Nổi bật</span>
          </label>
        ...
        <div className="flex gap-2.5 mt-6">
          <button onClick={onCancel} disabled={saving} className="flex-1 py-2.5 border border-white/10 text-fortress-silver/70 text-sm font-semibold hover:border-white/20 hover:text-fortress-ivory transition-all rounded-xl">
            Hủy
          </button>
          <button
            onClick={() => onSave(form)}
            disabled={saving}
            className="flex-1 py-2.5 bg-fortress-gold hover:bg-fortress-champagne text-fortress-navy text-sm font-bold transition-all rounded-xl disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {saving ? <div className="w-4 h-4 border-2 border-fortress-navy/30 border-t-fortress-navy rounded-full animate-spin" /> : null}
            {saving ? "Đang lưu…" : "Lưu"}
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#03080e] flex selection:bg-fortress-gold/20 selection:text-fortress-champagne font-sans">
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden min-h-screen relative">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 relative z-10 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h1 className="text-lg sm:text-xl font-bold text-fortress-ivory">Quản lý tài liệu</h1>
              <p className="text-fortress-silver/50 text-xs sm:text-sm mt-0.5">Báo cáo, công bố thông tin, điều lệ và đại hội cổ đông</p>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={load} className="p-2.5 border border-white/10 text-fortress-silver/50 hover:text-fortress-gold hover:border-fortress-gold/30 transition-all rounded-lg">
            ...
            <div className="px-5 py-4 border-b border-white/5 flex flex-col sm:flex-row items-start sm:items-center gap-3">
              <div className="relative flex-1 max-w-sm">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 bg-white/5 border border-white/10 text-fortress-ivory text-sm rounded-lg placeholder:text-fortress-silver/30 focus:outline-none focus:border-fortress-gold/40"
                />
              </div>
              <div className="flex items-center gap-1 bg-white/5 rounded-lg p-1 overflow-x-auto w-full sm:w-auto">
                <button
                  onClick={() => setFilterCat("all")}
                  className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${filterCat === "all" ? "bg-fortress-gold text-fortress-navy shadow" : "text-fortress-silver/50 hover:text-fortress-ivory"}`}
                >
                  Tất cả ({docs.length})
                </button>
                {CATEGORIES.map(([k, v]) => (
                  <button
                    ...
                    className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all whitespace-nowrap ${filterCat === k ? "bg-fortress-gold text-fortress-navy shadow" : "text-fortress-silver/50 hover:text-fortress-ivory"}`}
                  >
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20">
                <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center mb-4">
                  <FileText className="w-7 h-7 text-fortress-silver/20" />
                </div>
                <p className="text-fortress-ivory/60 font-medium mb-1">Không có tài liệu nào</p>
                <p className="text-fortress-silver/30 text-sm mb-5">Thêm tài liệu đầu tiên để bắt đầu.</p>
                <button
                  onClick={() => setFormTarget(defaultForm())}
                  className="px-4 py-2 bg-fortress-gold text-fortress-navy text-sm font-bold hover:bg-fortress-champagne transition-all rounded-lg"
                >
                  Thêm tài liệu
                </button>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/5">
                      <th className="text-left px-5 py-3 text-[10px] font-bold text-fortress-silver/40 uppercase tracking-wider">Tên tài liệu</th>
                      <th className="text-left px-4 py-3 text-[10px] font-bold text-fortress-silver/40 uppercase tracking-wider hidden sm:table-cell">Danh mục</th>
                      <th className="text-left px-4 py-3 text-[10px] font-bold text-fortress-silver/40 uppercase tracking-wider hidden md:table-cell">Năm</th>
                      <th className="text-left px-4 py-3 text-[10px] font-bold text-fortress-silver/40 uppercase tracking-wider hidden lg:table-cell">Ngày công bố</th>
                      <th className="text-center px-4 py-3 text-[10px] font-bold text-fortress-silver/40 uppercase tracking-wider hidden sm:table-cell">Trạng thái</th>
                      <th className="text-right px-4 py-3 text-[10px] font-bold text-fortress-silver/40 uppercase tracking-wider">Thao tác</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((doc) => (
                      <tr key={doc._id} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors group">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <div className="min-w-0">
                              <p className="font-medium text-fortress-ivory truncate max-w-[200px] sm:max-w-[280px] group-hover:text-fortress-gold transition-colors">
                                {doc.title}
                              </p>
                              <p className="text-[10px] text-fortress-silver/30 mt-0.5 uppercase">{doc.fileType}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-4 hidden sm:table-cell">
                          <span className="text-[10px] font-semibold px-2 py-0.5 bg-fortress-gold/10 text-fortress-gold border border-fortress-gold/20 rounded-full">
                            {CATEGORY_LABELS[doc.category]}
                          </span>
                        </td>
                        <td className="px-4 py-4 text-fortress-silver/50 text-sm hidden md:table-cell">{doc.year}{doc.quarter ? ` Q${doc.quarter}` : ""}</td>
                        <td className="px-4 py-4 text-fortress-silver/50 text-sm hidden lg:table-cell">{doc.publishedDate ? fmtDate(doc.publishedDate) : "—"}</td>
                        ...
                        <td className="px-4 py-4 text-right">
                          <div className="flex items-center justify-end gap-0.5">
                            <a href={doc.fileUrl} target="_blank" rel="noopener noreferrer" className="p-1.5 text-fortress-silver/30 hover:text-fortress-ivory transition-colors rounded-lg hover:bg-white/5" title="Xem">
                              <ExternalLink className="w-3.5 h-3.5" />
                            </a>
                            <button onClick={() => setFormTarget({ ...doc })} className="p-1.5 text-fortress-silver/30 hover:text-fortress-gold transition-colors rounded-lg hover:bg-fortress-gold/10" title="Sửa">
                              <Edit3 className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => setDeleteTarget(doc)} className="p-1.5 text-fortress-silver/30 hover:text-red-400 transition-colors rounded-lg hover:bg-red-500/10" title="Xóa">
            ...
            {filtered.length > 0 && (
              <div className="px-5 py-3 border-t border-white/5">
                <p className="text-xs text-fortress-silver/30">
                  Hiển thị <span className="text-fortress-ivory font-medium">{filtered.length}</span> / <span className="text-fortress-ivory font-medium">{docs.length}</span> tài liệu
                </p>
              </div>
            )}
          </div>
      ...
      <div className="fixed bottom-6 right-6 z-40">
        <a
          href="/documents"
          target="_blank"
          className="flex items-center gap-2 px-4 py-2.5 bg-[#07111D] border border-fortress-gold/30 text-fortress-gold text-xs font-semibold rounded-xl hover:bg-fortress-gold/10 transition-all shadow-xl"
        >
