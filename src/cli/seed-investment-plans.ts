/**
 * src/cli/seed-investment-plans.ts
 *
 * Seeds the database with default Investment Partnership Plans
 * aligned to NQ 5/2025/NQ-CP.
 *
 * Usage:
 *   npm run seed:plans              # skip if plans already exist
 *   npm run seed:plans -- --force   # drop existing plans and re-seed
 *
...
const PLANS = [
  {
    ...
    tagline:
      "Dành cho cá nhân muốn tham gia từ giai đoạn đầu — xây dựng nền tảng công nghệ tài chính số đầu tiên tại Việt Nam.",
    taglineEn:
      "For individuals joining from the ground up — building Vietnam's first digital financial technology platform.",
  ...
  },
  {
    ...
    tagline:
      "Phù hợp cho cá nhân chuyên nghiệp và doanh nghiệp nhỏ muốn tham gia giai đoạn triển khai thực tế.",
    taglineEn:
      "Ideal for professionals and small businesses joining during the deployment phase.",
  ...
  },
  {
    ...
    tagline:
      "Dành cho tổ chức và doanh nghiệp muốn tham gia giai đoạn mở rộng thị trường và scale nền tảng.",
    taglineEn:
      "For organizations joining during the market expansion and platform scaling phase.",
  ...
  },
  {
    ...
    tagline:
      "Dành cho định chế tài chính và đối tác chiến lược muốn đồng hành cùng tầm nhìn dài hạn.",
    taglineEn:
      "For financial institutions and strategic partners sharing a long-term vision.",
  ...
  },
  {
    ...
    tagline:
      "Gói đặc biệt dành cho đối tác neo — đóng vai trò định hình chiến lược và dẫn dắt hệ sinh thái tài chính số.",
    taglineEn:
      "Special package for anchor partners — shaping strategy and leading the digital financial ecosystem.",
...
async function run() {
  ...
  if (existing > 0 && !force) {
    console.log(
      `[!] Đã có ${existing} gói hợp tác. Bỏ qua seed (dùng --force để ghi đè).`,
    );
  ...
  if (existing > 0 && force) {
    console.log("[!] --force: Xóa các gói cũ...");
    await InvestmentPlan.deleteMany({});
  }

  console.log(`Đang seed ${PLANS.length} gói hợp tác...\n`);
  for (const plan of PLANS) {
    await InvestmentPlan.create(plan);
    console.log(`  [+] [${plan.tier}] ${plan.name} (${plan.status})`);
  }

  console.log("\n✓ Seed gói hợp tác hoàn thành.");
  await mongoose.disconnect();
}

run().catch((err) => {
  console.error("[ERROR] Seed thất bại:", err);
  process.exit(1);
});
