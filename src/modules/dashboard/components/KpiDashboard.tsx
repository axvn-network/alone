...
export interface KpiItem {
  ...
  icon: LucideIcon;
  /** Màu accent — tailwind text class, mặc định text-AXVN-gold */
  accentColor?: string;
}
...
function KpiCard({ item, index }: { item: KpiItem; index: number }) {
  const reduced = useReducedMotion();
  const accent = item.accentColor ?? "text-AXVN-gold";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-4">
        <span
          className={`flex h-10 w-10 items-center justify-center rounded-full bg-AXVN-gold/10 ${accent} group-hover:bg-AXVN-gold/20 transition-colors`}
        >
