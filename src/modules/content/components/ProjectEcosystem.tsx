      ...
      <div className="block lg:hidden space-y-3">
        ...
        <div className="flex justify-center">
          ...
          >
            ...
            <p className={`${SUB_SIZE.lg} text-AXVN-silver/50 mt-0.5`}>{NODES[0].sub}</p>
          </motion.div>
        </div>
        {/* Arrow down */}
        <div className="flex justify-center">
          <div className="w-px h-5 bg-AXVN-gold/30" />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {NODES.slice(1).map((node, i) => (
            ...
            >
              ...
              <p className={`${SUB_SIZE.sm} text-AXVN-silver/65 mt-1 leading-snug`}>{node.sub}</p>
            </motion.div>
          ))}
        </div>
        {/* Connections legend */}
        <div className="mt-4 space-y-2">
          {CONNECTIONS.map((c, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-AXVN-gold/50 shrink-0" />
              <p className="text-AXVN-silver/70 text-[11px]">
                <span className="text-AXVN-gold/80 font-semibold">AXVN Tech Holding</span>
                {" → "}
                <span className="text-AXVN-ivory/70">
            ...
            return (
              <g key={node.id}>
                <foreignObject x={x} y={y} width={w} height={h}>
                  ...
                  >
                    <p className={`font-black tracking-widest leading-tight ${node.size === "lg" ? "text-[11px]" : "text-[9.5px]"
                      } ${node.textColor}`}>
                      {node.label}
                    </p>
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-x-6 gap-y-2">
        {[
          ...
          <div key={item.label} className="flex items-center gap-1.5">
            ...
            <span className="text-AXVN-silver/70 text-[10px]">{item.label}</span>
          </div>
        ))}
      </div>
    </div>
