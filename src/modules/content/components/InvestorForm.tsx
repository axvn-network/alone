    ...
    return (
      <div className="text-center py-16">
        ...
        <p className="text-AXVN-silver/75 max-w-md mx-auto text-sm leading-relaxed">
          Đội ngũ AXVN Tech Holding sẽ phản hồi trong 2–3 ngày làm việc. Không có bot, không có mẫu tự động — người thật đọc và người thật trả lời.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {enquiryTypes.map((type) => (
            <label
              key={type.id}
              className={`flex items-start gap-3 p-4 border cursor-pointer transition-all duration-200 ${formData.enquiryType === type.id
            ...
            >
              ...
              />
              <div
                className={`w-9 h-9 flex items-center justify-center flex-shrink-0 mt-0.5 ${formData.enquiryType === type.id ? "bg-AXVN-gold/20" : "bg-white/5"
                  }`}
              >
                <type.icon
                  className={`w-4 h-4 ${formData.enquiryType === type.id ? "text-AXVN-gold" : "text-AXVN-silver"
                    }`}
                />
              </div>
              <div>
                <p
                  className={`font-medium text-sm ${formData.enquiryType === type.id ? "text-AXVN-gold" : "text-AXVN-ivory"
                    }`}
                >
                  {type.label}
                </p>
                <p className="text-AXVN-silver/70 text-xs mt-0.5">{type.description}</p>
              </div>
            </label>
          ))}
        </div>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <div>
          <label className="block text-AXVN-silver text-sm mb-2">Tên của bạn</label>
          <input
            type="text"
            name="name"
            value={formData.name}
        ...
        <div>
          <label className="block text-AXVN-silver text-sm mb-2">Số điện thoại</label>
          <input
            type="tel"
            name="phone"
        ...
        <div className="flex flex-wrap gap-2">
          {INVESTMENT_RANGES.map((r) => (
            <button
              ...
              className={`px-4 py-2 text-sm border transition-all duration-150 rounded-sm ${formData.investmentRange === r.value
