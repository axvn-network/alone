...
export default function WhatWeDo() {
  ...
  const i18nCards = (t("whatWeDo.cards", lang) as unknown as { title: string; desc: string }[] | string);
  const fallbackCards = Array.isArray(i18nCards) ? i18nCards : [];
  const cardList = (content.whatCards && Array.isArray(content.whatCards) && content.whatCards.length > 0)
    ? content.whatCards
    : fallbackCards;
          ...
          <StaggerItem>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-14 mb-8 md:mb-14">
              <div>
                ...
                <p className="text-AXVN-gold/90 font-medium mt-4 leading-[1.8]" style={{ fontSize: "var(--text-body)" }}>
                  {subtitle}
                </p>
              </div>
              <div className="space-y-5 lg:self-end">
                <p className="text-AXVN-silver/80 leading-[1.8]" style={{ fontSize: "var(--text-body)" }}>{desc1}</p>
                <p className="text-AXVN-silver/75 leading-[1.8]" style={{ fontSize: "var(--text-body)" }}>{desc2}</p>
              </div>
            </div>
          </StaggerItem>
              ...
              {cardList.map((item, i) => (
                ...
                >
                  <div className="flex-1 flex flex-col">
                    ...
                    <p className="text-AXVN-silver/70 leading-[1.8] flex-1" style={{ fontSize: "var(--text-body)" }}>
                      {item.desc}
                    </p>
                    <motion.div
