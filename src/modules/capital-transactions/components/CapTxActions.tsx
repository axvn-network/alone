...
export function CapCallModal({ shareholders, onClose }: CapCallModalProps) {
  ...
  const [form, setForm] = useState({
    shareholderId: "", amount: "", description: "", referenceNo: "", adminNote: "",
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-[#03080e] border border-AXVN-gold/20 rounded-2xl shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-AXVN-silver/60 text-xs mb-1.5">Cổ đông *</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-AXVN-silver/60 text-xs mb-1.5">Số tiền (VNĐ) *</label>
            ...
            <div>
              <label className="block text-AXVN-silver/60 text-xs mb-1.5">Đợt / Mã</label>
              <input type="text"
          ...
          <div>
            <label className="block text-AXVN-silver/60 text-xs mb-1.5">Mô tả</label>
            <input type="text"
          ...
          <div>
            <label className="block text-AXVN-silver/60 text-xs mb-1.5">Ghi chú nội bộ</label>
            <input type="text"
...
export function ReviewModal({ tx, onClose }: ReviewModalProps) {
  ...
  function handleAction(status: "confirmed" | "rejected") {
    startTransition(async () => {
      const result = await updateCapTxStatusAction({ id: tx._id, status, adminNote });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-md bg-[#03080e] border border-AXVN-gold/20 rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between p-6 border-b border-AXVN-gold/10">
          <h2 className="text-AXVN-ivory font-bold text-sm">Xét duyệt chuyển khoản</h2>
          <button onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-AXVN-silver hover:text-white transition-colors text-xs">
            ✕
          </button>
        ...
        <div className="p-6 space-y-4">
          <div className="bg-AXVN-navy rounded-xl p-4 text-xs text-AXVN-silver/60 space-y-1">
            <p><span className="text-AXVN-silver/40">Cổ đông:</span>{" "}
              <span className="text-AXVN-ivory">{tx.shareholderName}</span></p>
            <p><span className="text-AXVN-silver/40">Số tiền:</span>{" "}
              <span className="text-AXVN-gold font-bold">{formatVNDCompact(tx.amount)} {tx.currency}</span></p>
            {tx.referenceNo && <p><span className="text-AXVN-silver/40">Ref:</span> {tx.referenceNo}</p>}
            {tx.description && <p><span className="text-AXVN-silver/40">Mô tả:</span> {tx.description}</p>}
            {tx.proofUrl && (
              <p>
          ...
          <div>
            <label className="block text-AXVN-silver/60 text-xs mb-1.5">Ghi chú admin</label>
          ...
          <p className="text-AXVN-gold/50 text-[11px] leading-relaxed">
            ⚠️ Xác nhận sẽ cộng{" "}
            <strong>{formatVNDCompact(tx.amount)} VNĐ</strong>{" "}
            vào <code>capitalPaid</code> của cổ đông này.
          </p>

          <div className="flex gap-3">
            <button onClick={() => handleAction("confirmed")} disabled={pending}
            ...
            <button onClick={() => handleAction("rejected")} disabled={pending}
