  ...
  return (
    <>
      ...
      <div className="bg-[#03080e] border border-AXVN-gold/10 rounded-2xl overflow-hidden">
        {txs.length === 0 ? (
          <p className="text-center text-AXVN-silver/40 py-16 text-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {txs.map((tx) => {
                  const cfg =
                    CAPITAL_TX_STATUS_CONFIG[tx.status] ??
                    CAPITAL_TX_STATUS_CONFIG.pending;
                  const CfgIcon = cfg.icon;
                  return (
                    <tr
                      key={tx._id}
                      className="border-b border-AXVN-gold/5 hover:bg-white/2 transition-colors"
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        ...
                        <span className="text-AXVN-silver/30 text-[10px] ml-1">
                          {tx.currency}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full font-semibold border ${cfg.cls}`}
                        >
      ...
      {reviewTx && (
        <ReviewModal tx={reviewTx} onClose={() => setReviewTx(null)} />
      )}
    </>
  );
}
