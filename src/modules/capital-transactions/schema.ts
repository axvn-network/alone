...
export const capTxTypeEnum = z.enum(
  ["capital_call", "deposit", "payment_confirm", "adjustment"] as [CapTxType, ...CapTxType[]]
);

export const capTxStatusEnum = z.enum(
  ["pending", "confirmed", "rejected", "cancelled"] as [CapTxStatus, ...CapTxStatus[]]
);
...
export const updateCapTxStatusSchema = z.object({
  ...
  status:    z.enum(["confirmed", "rejected", "cancelled"] as ["confirmed", "rejected", "cancelled"]),
  adminNote: z.string().max(1000).optional().default(""),
});
