...
export function AccordionRoot({
  ...
  return (
    <Ctx.Provider value={{ openIds, toggle }}>
      <div className={`divide-y divide-gvi-gold/10 rounded-xl border border-gvi-gold/15 overflow-hidden ${className}`}>
        {children}
      </div>
    </Ctx.Provider>
  );
  ...
  return (
    <div className={className}>
      ...
      <button
        ...
        className="w-full flex items-center justify-between gap-3 px-5 py-4 text-left bg-gvi-deep hover:bg-gvi-navy/50 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gvi-gold"
      >
        <span className="flex items-center gap-3 min-w-0">
          <span className="text-sm font-semibold text-gvi-ivory leading-snug">{title}</span>
          {badge && (
            <span className="text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-gvi-gold/10 text-gvi-gold shrink-0">
              {badge}
            </span>
          )}
        </span>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="px-5 py-4 bg-gvi-navy/30 text-gvi-silver/80 text-sm leading-relaxed">
              {children}
            </div>
          </motion.div>
        )}
