...
export default function ContactForm() {
  ...
  const inputCls = "w-full px-4 md:px-5 py-3 md:py-3.5 bg-gvi-ivory border border-gvi-navy/20 text-gvi-navy placeholder:text-gvi-silver focus:outline-none focus-visible:outline-none focus:border-gvi-gold focus:ring-2 focus:ring-gvi-gold/20 hover:border-gvi-navy/30 transition-all duration-200 rounded-lg";
  const labelCls = "block text-gvi-navy text-[11px] font-semibold uppercase tracking-[0.18em] mb-1.5";

  return (
    <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6">
      {/* Type Selector */}
      <div className="flex gap-4 p-1 bg-gvi-ivory border border-gvi-navy/10 rounded-lg">
        <button
          type="button"
          className={`flex-1 py-2 text-xs font-bold uppercase tracking-widest transition-all ${
            formData.type === "Contact" ? "bg-gvi-gold text-gvi-navy rounded shadow" : "text-gvi-silver"
          }`}
        ...
        <button
          type="button"
          className={`flex-1 py-2 text-xs font-bold uppercase tracking-widest transition-all ${
            formData.type === "Scheduling" ? "bg-gvi-gold text-gvi-navy rounded shadow" : "text-gvi-silver"
          }`}
      ...
      {formData.type === "Scheduling" && (
        <div className="grid sm:grid-cols-2 gap-4 md:gap-5 animate-in fade-in slide-in-from-top-2 duration-300">
          <div>
            <label className={labelCls}>Ngày Mong Muốn <span className="text-gvi-gold">*</span></label>
            <input
              type="date"
              name="desiredDate"
          ...
          <div>
            <label className={labelCls}>Khung Giờ <span className="text-gvi-gold">*</span></label>
            <select
              name="desiredTime"
      ...
      {/* Consent checkbox */}
      <div className="flex items-start gap-3 p-4 bg-gvi-navy/5 border border-gvi-navy/10 rounded-lg">
        <input
          type="checkbox"
          id="consentGiven"
          name="consentGiven"
          ...
          onChange={handleChange}
          className="mt-0.5 w-4 h-4 accent-gvi-gold cursor-pointer shrink-0"
          required
        />
        <label htmlFor="consentGiven" className="text-gvi-navy text-xs leading-relaxed cursor-pointer">
          <span className="text-gvi-gold font-semibold">*</span> Tôi đồng ý để GVI Tech Holding xử lý thông tin cá nhân của tôi nhằm mục đích phản hồi yêu cầu liên hệ này theo{" "}
          <a href="/privacy-policy" target="_blank" className="text-gvi-navy underline hover:text-gvi-gold transition-colors">Chính sách Bảo mật</a>{" "}
      ...
      <button
        ...
        className="w-full sm:w-auto px-9 md:px-11 py-3.5 md:py-4 bg-gvi-gold hover:bg-gvi-champagne active:scale-[0.98] text-gvi-navy font-bold text-xs tracking-[0.18em] uppercase transition-all duration-200 flex items-center justify-center gap-2 shadow-lg shadow-gvi-gold/15 focus-visible:outline-2 focus-visible:outline-gvi-gold focus-visible:outline-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {loading ? (
          <>
