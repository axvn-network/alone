...
const QUESTIONS: QuizQuestion[] = [
  ...
  {
    id: "q2",
    question: "Nếu tham gia GVI Tech Holding, bạn có thể đóng góp năng lực kỹ thuật cụ thể nào?",
    options: [
  ...
  {
    id: "q13",
    question: "Bạn có thể cam kết tham gia chủ động vào hoạt động của GVI Tech Holding ở mức độ nào?",
    options: [
  ...
  {
    id: "q14",
    question: "Điều gì thu hút bạn nhất khi cân nhắc tham gia GVI Tech Holding?",
    options: [
  ...
  if (step === 0) {
    return (
      <div className="text-center py-8">
        <div className="w-16 h-16 bg-gvi-gold/10 border border-gvi-gold/30 rounded-full flex items-center justify-center mx-auto mb-5">
          <span className="text-gvi-gold font-bold text-xl">?</span>
        </div>
        <h3 className="text-gvi-ivory font-bold text-lg mb-3">Đánh Giá Vai Trò Phù Hợp</h3>
        <p className="text-gvi-silver/75 text-sm leading-relaxed max-w-md mx-auto mb-6">
        ...
        <button
          onClick={() => setStep(1)}
          className="inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-gvi-gold to-gvi-champagne text-gvi-navy font-bold text-sm hover:opacity-90 transition-opacity rounded-sm"
        >
          Bắt Đầu Đánh Giá
          <ChevronRight className="w-4 h-4" />
        </button>
        <p className="text-gvi-silver/40 text-xs mt-3">Khoảng 3–5 phút · Hoàn toàn bảo mật</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <CheckCircle2 className="w-10 h-10 text-gvi-gold mx-auto mb-3" />
          <h3 className="text-gvi-ivory font-bold text-lg mb-1">Kết Quả Đánh Giá</h3>
          <p className="text-gvi-silver/70 text-sm">Dựa trên câu trả lời của bạn, hệ thống gợi ý:</p>
        </div>

        <div className="bg-gvi-gold/8 border border-gvi-gold/30 rounded-xl p-5 text-center">
          <p className="text-gvi-silver/60 text-xs tracking-widest uppercase mb-1">Vai Trò Đề Xuất</p>
          <p className="text-gvi-gold font-bold text-xl">{ROLE_LABELS[result.suggestedRole]}</p>
        </div>
          ...
          {dims.sort(([, a], [, b]) => b - a).map(([dim, score]) => (
            <div key={dim}>
              <div className="flex items-center justify-between mb-1">
                <span className="text-gvi-silver text-xs font-medium">{DIM_LABELS[dim]}</span>
                <span className="text-gvi-gold text-xs font-mono">{score}</span>
              </div>
              <div className="h-1.5 bg-gvi-deep rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-gvi-gold to-gvi-champagne rounded-full transition-all duration-700"
                  style={{ width: `${score}%` }}
                />
              </div>
            </div>
        ...
        <div className="flex gap-3 pt-2">
          <button
            ...
            className="flex-1 py-3 border border-gvi-gold/20 text-gvi-silver text-sm hover:border-gvi-gold/40 hover:text-gvi-ivory transition-all rounded-sm"
          >
            Làm Lại
          </button>
          <button
            onClick={() => onComplete(result)}
            className="flex-[2] py-3 bg-gradient-to-r from-gvi-gold to-gvi-champagne text-gvi-navy font-bold text-sm hover:opacity-90 transition-opacity rounded-sm flex items-center justify-center gap-2"
          >
            Tiếp Tục Đăng Ký
            <ChevronRight className="w-4 h-4" />
          </button>
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <span className="text-gvi-silver/60 text-xs">Câu {step}/{totalQuestions}</span>
          <span className="text-gvi-gold text-xs font-mono">{progress}%</span>
        </div>
        <div className="h-1 bg-gvi-deep rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-gvi-gold to-gvi-champagne rounded-full transition-all duration-300"
      ...
      {/* Câu hỏi */}
      <div>
        <p className="text-gvi-ivory font-semibold text-sm leading-relaxed">{q.question}</p>
      </div>
          ...
          <button
            ...
            className={`w-full text-left p-4 border transition-all duration-150 rounded-sm ${
              answers[q.id] === opt.value
                ? "border-gvi-gold/60 bg-gvi-gold/8 text-gvi-ivory"
                : "border-gvi-gold/10 bg-gvi-deep/40 text-gvi-silver hover:border-gvi-gold/30 hover:text-gvi-ivory"
            }`}
          >
            <div className="flex items-start gap-3">
              <span className={`shrink-0 w-5 h-5 rounded-full border flex items-center justify-center text-[10px] font-bold mt-0.5 ${
                answers[q.id] === opt.value
                  ? "bg-gvi-gold border-gvi-gold text-gvi-navy"
                  : "border-gvi-gold/20 text-gvi-silver/50"
              }`}>
      ...
      {qIndex > 0 && (
        <button
          ...
          className="flex items-center gap-1.5 text-gvi-silver/50 hover:text-gvi-silver text-xs transition-colors"
        >
